...
CLI := build/turingchain-cli
SRC_CLI := github.com/turingchain2020/plugin/cli
APP := build/turingchain
BUILD_FLAGS = -ldflags "-X github.com/turingchain2020/turingchain/common/version.GitCommit=`git rev-parse --short=8 HEAD`"
LDFLAGS := -ldflags "-w -s"
...
build: depends
	...
	go build $(BUILD_FLAGS) -v -i -o build/fork-config github.com/turingchain2020/plugin/cli/fork_config/
	@cp turingchain.toml build/
	@cp turingchain.para.toml build/ci/paracross/
...
update: ## version 可以是git tag打的具体版本号,也可以是commit hash, 什么都不填的情况下默认从master分支拉取最新版本
	@if [ -n "$(version)" ]; then   \
	go get -v github.com/turingchain2020/turingchain@${version}  ; \
	else \
	go get -v github.com/turingchain2020/turingchain@master ;fi
	@go mod tidy
dep:
...
docker: ## build docker image for turingchain run
	@sudo docker build . -f ./build/Dockerfile-run -t turingchain:latest

#extra can make more test setting
docker-compose: ## build docker-compose for turingchain run
	@cd build && if ! [ -d ci ]; then \
	 make -C ../ ; \
	 fi; \
	 cp turingchain* Dockerfile  docker-compose.yml *.sh ci/ && cd ci/ && ./docker-compose-pre.sh run $(proj) $(dapp) $(extra) && cd ../..

docker-compose-down: ## build docker-compose for turingchain run
	@cd build && if [ -d ci ]; then \
	 cp turingchain* Dockerfile  docker-compose* ci/ && cd ci/ && ./docker-compose-pre.sh down $(proj) $(dapp) && cd .. ; \
	 fi; \
	 cd ..

metrics:## build docker-compose for turingchain metrics
	@cd build && if ! [ -d ci ]; then \
	 make -C ../ ; \
	 fi; \
	 cp turingchain* Dockerfile  docker-compose.yml docker-compose-metrics.yml influxdb.conf *.sh ci/paracross/testcase.sh metrics/ && ./docker-compose-pre.sh run $(proj) metrics  && cd ../..


fork-test: ## build fork-test for turingchain run
	@cd build && cp turingchain* Dockerfile system-fork-test.sh docker-compose* ci/ && cd ci/ && ./docker-compose-pre.sh forktest $(proj) $(dapp) && cd ../..
...
#	@cd ${TURINGCHAIN_PATH}/types/proto && ./create_protobuf.sh && cd ../..
...
addupstream:
	git remote add upstream https://github.com/turingchain2020/plugin.git
	git remote -v
