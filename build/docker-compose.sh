...
NODE3="${1}_turingchain_1"
CLI="docker exec ${NODE3} /root/turingchain-cli"
...
NODE4="${1}_chain30_1"
#CLI4="docker exec ${NODE4} /root/turingchain-cli"

NODE5="${1}_chain29_1"
CLI5="docker exec ${NODE5} /root/turingchain-cli"
...
function base_init() {
    ...
    sed -i $sedfix 's/^Title.*/Title="local"/g' turingchain.toml
    sed -i $sedfix 's/^TestNet=.*/TestNet=true/g' turingchain.toml

    sed -i $sedfix 's/^powLimitBits=.*/powLimitBits="0x1f2fffff"/g' turingchain.toml
    sed -i $sedfix 's/^targetTimePerBlock=.*/targetTimePerBlock=1/g' turingchain.toml
    sed -i $sedfix 's/^targetTimespan=.*/targetTimespan=10000000/g' turingchain.toml
    sed -i $sedfix 's/^isLevelFee=.*/isLevelFee=false/g' turingchain.toml

    # p2p
    sed -i $sedfix '0,/^seeds=.*/s//seeds=["turingchain:13802","chain32:13802","chain31:13802"]/g' turingchain.toml
    #sed -i $sedfix 's/^enable=.*/enable=true/g' turingchain.toml
    sed -i $sedfix '0,/^enable=.*/s//enable=true/' turingchain.toml
    sed -i $sedfix 's/^isSeed=.*/isSeed=true/g' turingchain.toml
    sed -i $sedfix 's/^innerSeedEnable=.*/innerSeedEnable=false/g' turingchain.toml
    sed -i $sedfix 's/^useGithub=.*/useGithub=false/g' turingchain.toml

    # rpc
    sed -i $sedfix 's/^jrpcBindAddr=.*/jrpcBindAddr="0.0.0.0:9671"/g' turingchain.toml
    sed -i $sedfix 's/^grpcBindAddr=.*/grpcBindAddr="0.0.0.0:9672"/g' turingchain.toml
    sed -i $sedfix 's/^whitelist=.*/whitelist=["localhost","127.0.0.1","0.0.0.0"]/g' turingchain.toml

    # wallet
    sed -i $sedfix 's/^minerdisable=.*/minerdisable=false/g' turingchain.toml

    sed -i $sedfix 's/^nodeGroupFrozenCoins=.*/nodeGroupFrozenCoins=20/g' turingchain.toml
    sed -i $sedfix 's/^paraConsensusStopBlocks=.*/paraConsensusStopBlocks=100/g' turingchain.toml

    # blockchain
    # TODO 剩下evm trade 测试和这个选项有关，在其他pr中解决，不使得这个pr太大
    sed -i $sedfix 's/^enableReduceLocaldb=.*/enableReduceLocaldb=false/g' turingchain.toml
    sed -i $sedfix 's/^enablePushSubscribe=.*/enablePushSubscribe=true/g' turingchain.toml

    # ticket
    sed -i $sedfix 's/^ticketPrice =.*/ticketPrice = 10000/g' turingchain.toml

    #relay genesis
    sed -i $sedfix 's/^genesis="12qyocayNF7.*/genesis="1G5Cjy8LuQex2fuYv3gzb7B8MxAnxLEqt3"/g' turingchain.toml

    #autonomy
    sed -i $sedfix 's/^useBalance=.*/useBalance=true/g' turingchain.toml
    sed -i $sedfix 's/^total="16htvcBNS.*/total="1Q9sQwothzM1gKSzkVZ8Dt1tqKX1uzSagx"/g' turingchain.toml
...
function start() {
    ...
    if [ -n "$influxdbcontainer" ]; then
        echo "create database turingchainmetrics in docker container build_influxdb_1"
        docker exec build_influxdb_1 influx -execute 'create database turingchainmetrics'
    fi
    set -e
...
function check_docker_status() {
    status=$(docker-compose ps | grep turingchain_1 | awk '{print $6}')
    statusPara=$(docker-compose ps | grep turingchain_1 | awk '{print $3}')
    if [ "${status}" == "Exit" ] || [ "${statusPara}" == "Exit" ]; then
        echo "=========== turingchain service Exit logs ========== "
        docker-compose logs turingchain
        echo "=========== turingchain service Exit logs End========== "
    fi
...
function rpc_test() {
    if [ "$DAPP" == "" ]; then
        system_test_rpc "http://${1}:9671"
        dapp_test_address "${CLI}"
        dapp_test_rpc "http://${1}:9671" "${dockerNamePrefix}"
    fi
