...
function main() {
    if [ "${OP}" == "run" ]; then
        #copy turingchain system-test-rpc.sh
        cp "$(go list -f "{{.Dir}}" github.com/turingchain2020/turingchain)"/build/system-test-rpc.sh ./
        ...
        sed -i $sedfix '/^useGithub=.*/a version=1' turingchain.toml
    fi
}
