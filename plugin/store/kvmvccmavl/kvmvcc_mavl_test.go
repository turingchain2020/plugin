...
import (
	...
	drivers "github.com/turingchain2020/turingchain/system/store"
...
func TestIterateRangeByStateHash(t *testing.T) {
	...
	accCoin := account.NewCoinsAccount(types.NewTuringchainConfigNoInit(types.GetDefaultCfgstring()))
	...
	store.IterateRangeByStateHash(hash, []byte("mavl-coins-trc-"), []byte("mavl-coins-trc-exec"), true, resp.IterateRangeByStateHash)
	...
	resp = &types.ReplyGetTotalCoins{}
	resp.Count = 100000
	store.IterateRangeByStateHash(hash1, []byte("mavl-coins-trc-"), []byte("mavl-coins-trc-exec"), true, resp.IterateRangeByStateHash)
	fmt.Println("resp.Num=", resp.Num)
	fmt.Println("resp.Amount=", resp.Amount)
	...
	resp = &types.ReplyGetTotalCoins{}
	resp.Count = 100000
	store.IterateRangeByStateHash(hash1, []byte("mavl-coins-trc-06htvcBNSEA7fZhAdLJphDwQRQJaHpy003"), []byte("mavl-coins-trc-exec"), true, resp.IterateRangeByStateHash)
	fmt.Println("resp.Num=", resp.Num)
	fmt.Println("resp.Amount=", resp.Amount)
	...
	resp = &types.ReplyGetTotalCoins{}
	resp.Count = 10000
	store.IterateRangeByStateHash(firstForkHash, []byte("mavl-coins-trc-"), []byte("mavl-coins-trc-exec"), true, resp.IterateRangeByStateHash)
	fmt.Println("resp.Num=", resp.Num)
	fmt.Println("resp.Amount=", resp.Amount)
