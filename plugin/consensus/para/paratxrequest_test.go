...
func TestCalcCommitMsgTxs(t *testing.T) {
	cfg := types.NewTuringchainConfig(testnode.DefaultConfig)
...
//	turingchainCfg := types.NewTuringchainConfig(testnode.DefaultConfig)
//
//	api := new(apimocks.QueueProtocolAPI)
//	api.On("GetConfig", mock.Anything).Return(turingchainCfg, nil)
//	para := &client{BaseClient: &drivers.BaseClient{}}
//
//	para.subCfg = new(subConfig)
