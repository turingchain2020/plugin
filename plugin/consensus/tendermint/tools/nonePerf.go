...
import (
	...
	ty "github.com/turingchain2020/plugin/plugin/dapp/valnode/types"
...
func main() {
	...
	fmt.Println("jrpc url:", os.Args[2]+":9671")
...
func Perf(host, txsize, num, sleepinterval, totalduration string) {
	...
	go func() {
		...
		defer conn.Close()
		gcli := types.NewTuringchainClient(conn)
		for {
...
func getHeight(gcli types.TuringchainClient) (int64, error) {
...
func Put(ip string, size string, privkey string) {
	...
	poststr := fmt.Sprintf(`{"jsonrpc":"2.0","id":2,"method":"Turingchain.SendTransaction","params":[{"data":"%v"}]}`,
...
func Get(ip string, hash string) {
	...
	poststr := fmt.Sprintf(`{"jsonrpc":"2.0","id":2,"method":"Turingchain.QueryTransaction","params":[{"hash":"%s"}]}`, hash)
	resp, err := http.Post(url, "application/json", bytes.NewBufferString(poststr))
	if err != nil {
		fmt.Println(err)
		return
...
func setTxHeight(ip string) {
	url := "http://" + ip + ":9671"
	poststr := fmt.Sprintf(`{"jsonrpc":"2.0","id":2,"method":"Turingchain.GetLastHeader","params":[]}`)
	resp, err := http.Post(url, "application/json", bytes.NewBufferString(poststr))
	if err != nil {
		fmt.Println(err)
		return
