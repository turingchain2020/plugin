...
func testTicket(t *testing.T) {
	mock33 := testnode.New("testdata/turingchain.cfg.toml", nil)
	defer mock33.Close()
...
func createBindMiner(cfg *types.TuringchainConfig, t *testing.T, m, r string, priv crypto.PrivKey) *types.Transaction {
...
func Test_getNextRequiredDifficulty(t *testing.T) {
	cfg := types.NewTuringchainConfig(types.ReadFile("testdata/turingchain.cfg.toml"))
