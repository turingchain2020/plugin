...
import (
	...
	"github.com/turingchain2020/turingchain/common/difficulty"
	"github.com/turingchain2020/turingchain/common/log/log15"
	vrf "github.com/turingchain2020/turingchain/common/vrf/secp256k1"
	"github.com/turingchain2020/turingchain/queue"
	drivers "github.com/turingchain2020/turingchain/system/consensus"
	driver "github.com/turingchain2020/turingchain/system/dapp"
...
//316190000 coins
func createTicket(cfg *types.TuringchainConfig, minerAddr, returnAddr string, count int32, height int64) (ret []*types.Transaction) {
...
func (client *Client) CheckBlock(parent *types.Block, current *types.BlockDetail) error {
	turingchainCfg := client.GetAPI().GetConfig()
	cfg := ty.GetTicketMinerParam(turingchainCfg, current.Block.Height)
	...
	//vrf verify
	if turingchainCfg.IsDappFork(current.Block.Height, ty.TicketX, "ForkTicketVrf") {
		var input []byte
...
func (client *Client) getNextRequiredDifficulty(block *types.Block, bits uint32) (uint32, []byte, error) {
	...
	// Genesis block.
	if block == nil {
		return turingchainCfg.GetP(0).PowLimitBits, defaultModify, nil
	}
	powLimitBits := turingchainCfg.GetP(block.Height).PowLimitBits
	cfg := ty.GetTicketMinerParam(turingchainCfg, block.Height)
...
func (client *Client) updateBlock(block *types.Block, txHashList [][]byte) (*types.Block, *types.Block, [][]byte) {
	...
	newblock.ParentHash = lastBlock.Hash(turingchainCfg)
	newblock.Height = lastBlock.Height + 1
	cfg := turingchainCfg.GetP(newblock.Height)
