...
import (
	...
	executor "github.com/turingchain2020/plugin/plugin/dapp/ticket/executor"
	pty "github.com/turingchain2020/plugin/plugin/dapp/ticket/types"
)
...
func Test_Exec_Bind_Unbind(t *testing.T) {
	turingchainTestCfg := mock33.GetAPI().GetConfig()
	env := execEnv{
		1539918074,
		10000,
		2,
	...
	bindTx := createBindMiner(t, turingchainTestCfg, string(Nodes[1]), string(Nodes[0]), priv)
	...
	unbindTx := createBindMiner(t, turingchainTestCfg, "", string(Nodes[0]), priv)
