...
"'${boardsAddr[20]}'"
'
turingchain_para_init() {
    ip=$1
    turingchain_ImportPrivkey "${votePrKey}" "${voteAddr}" "autonomytest" "${ip}"
    turingchain_SendToAddress "12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv" "$voteAddr" 630000000000 "${ip}"
}
turingchain_applyCoinsNOLimit() {
...
handleBoards() {
    ...
    for ((i = 0; i < ${#boardsPrKey[*]}; i++)); do
        echo "${boardsPrKey[$i]}"
        lab="board_"${i}
        turingchain_ImportPrivkey "${boardsPrKey[$i]}" "${boardsAddr[$i]}" "${lab}" "${ip}"
        turingchain_applyCoins "${boardsAddr[$i]}" 100000000 "${ip}"
    done
}

txQuery() {
    ty=$(curl -ksd '{"method":"Turingchain.QueryTransaction","params":[{"hash":"'"$RAW_TX_HASH"'"}]}' "${HTTP}" | jq -r ".result.receipt.ty")
    if [[ ${ty} != 2 ]]; then
        txQueryShow=$(curl -ksd '{"method":"Turingchain.QueryTransaction","params":[{"hash":"'"$RAW_TX_HASH"'"}]}' "${HTTP}" | jq -r ".result")
        echo "$txQueryShow"
...
proposalBoardTx() {
    local start=$1
    local end=$2
    local req='{"method":"Turingchain.CreateTransaction","params":[{"execer":"'"${EXECTOR}"'", "actionName":"PropBoard", "payload":{"boards": ['"${boards}"'],"startBlockHeight":'"${start}"',"endBlockHeight":'"${end}"'}}]}'
    echo "${req}"
    turingchain_Http "$req" ${HTTP} '(.error|not) and (.result != null)' "$FUNCNAME" ".result"
    turingchain_SignAndSendTx "${RETURN_RESP}" "${propKey}" "${HTTP}"
...
voteBoardTx() {
    local ID=$1
    local privk=$2
    local req='{"method":"Turingchain.CreateTransaction","params":[{"execer":"'"${EXECTOR}"'", "actionName":"VotePropBoard", "payload":{"proposalID": "'"${ID}"'","approve": true}}]}'
    echo "${req}"
    turingchain_Http "$req" ${HTTP} '(.error|not) and (.result != null)' "$FUNCNAME" ".result"
    turingchain_SignAndSendTx "${RETURN_RESP}" "${privk}" "${HTTP}"
    echo "$RAW_TX_HASH"
    txQuery "$FUNCNAME"
}
...
revokeProposalTx() {
    local ID=$1
    local funcName=$2
    local req='{"method":"Turingchain.CreateTransaction","params":[{"execer":"'"${EXECTOR}"'", "actionName":"'"${funcName}"'", "payload":{"proposalID": "'"${ID}"'"}}]}'
    echo "${req}"
    turingchain_Http "$req" ${HTTP} '(.error|not) and (.result != null)' "$FUNCNAME" ".result"
    turingchain_SignAndSendTx "${RETURN_RESP}" "${propKey}" "${HTTP}"
    echo "$RAW_TX_HASH"
...
queryProposal() {
    local ID=$1
    local funcName=$2
    local req='{"method":"Turingchain.Query","params":[{"execer":"'"${EXECTOR}"'","funcName":"'"${funcName}"'","payload":{"data":"'"${ID}"'"}}]}'
    resok='(.error|not)'
    turingchain_Http "$req" ${HTTP} "$resok" "$FUNCNAME"
}
...
listProposal() {
    ...
    local addr=""
    local direct=0
    local req='{"method":"Turingchain.Query","params":[{"execer":"'"${EXECTOR}"'","funcName":"'"${funcName}"'","payload":{"status":"'"${status}"'", "addr":"'"${addr}"'", "count":1, "direction":"'"${direct}"'"}}]}'
    resok='(.error|not)'
    turingchain_Http "$req" ${HTTP} "$resok" "$FUNCNAME"
}

queryActivePropBoard() {
    local req='{"method":"Turingchain.Query","params":[{"execer":"'"${EXECTOR}"'","funcName":"GetActiveBoard","payload":{"data":"1"}}]}'
    resok='(.error|not)'
    turingchain_Http "$req" ${HTTP} "$resok" "$FUNCNAME"
}
...
testProposalBoard() {
    #proposal
    turingchain_LastBlockHeight ${HTTP}
    start=$((LAST_BLOCK_HEIGHT + 100))
    end=$((start + 20 + 720))
    proposalBoardTx ${start} ${end}
    #vote
    turingchain_BlockWait 100 "$HTTP"
...
proposalRuleTx() {
    ...
    local end=$2
    local propAmount=$3
    local req='{"method":"Turingchain.CreateTransaction","params":[{"execer":"'"${EXECTOR}"'", "actionName":"PropRule", "payload":{"ruleCfg": {"proposalAmount" : '"${propAmount}"'},"startBlockHeight":'"${start}"',"endBlockHeight":'"${end}"'}}]}'
    echo "${req}"
    turingchain_Http "$req" ${HTTP} '(.error|not) and (.result != null)' "$FUNCNAME" ".result"
    turingchain_SignAndSendTx "${RETURN_RESP}" "${propKey}" "${HTTP}"
...
voteRuleTx() {
    local ID=$1
    local privk=$2
    local req='{"method":"Turingchain.CreateTransaction","params":[{"execer":"'"${EXECTOR}"'", "actionName":"VotePropRule", "payload":{"proposalID": "'"${ID}"'","approve": true}}]}'
    echo "${req}"
    turingchain_Http "$req" ${HTTP} '(.error|not) and (.result != null)' "$FUNCNAME" ".result"
    turingchain_SignAndSendTx "${RETURN_RESP}" "${privk}" "${HTTP}"
    echo "$RAW_TX_HASH"
...
queryActivePropRule() {
    local req='{"method":"Turingchain.Query","params":[{"execer":"'"${EXECTOR}"'","funcName":"GetActiveRule","payload":{"data":"1"}}]}'
    resok='(.error|not)'
    turingchain_Http "$req" ${HTTP} "$resok" "$FUNCNAME"
}
...
proposalProjectTx() {
    ...
    local amount=$3
    local toAddr=$4
    local req='{"method":"Turingchain.CreateTransaction","params":[{"execer":"'"${EXECTOR}"'", "actionName":"PropProject", "payload":{"amount" : '"${amount}"', "toAddr" : "'"${toAddr}"'","startBlockHeight":'"${start}"',"endBlockHeight":'"${end}"'}}]}'
    echo "${req}"
    turingchain_Http "$req" ${HTTP} '(.error|not) and (.result != null)' "$FUNCNAME" ".result"
    turingchain_SignAndSendTx "${RETURN_RESP}" "${propKey}" "${HTTP}"
...
voteProjectTx() {
    local ID=$1
    local privk=$2
    local req='{"method":"Turingchain.CreateTransaction","params":[{"execer":"'"${EXECTOR}"'", "actionName":"VotePropProject", "payload":{"proposalID": "'"${ID}"'","approve": true}}]}'
    echo "${req}"
    turingchain_Http "$req" ${HTTP} '(.error|not) and (.result != null)' "$FUNCNAME" ".result"
    turingchain_SignAndSendTx "${RETURN_RESP}" "${privk}" "${HTTP}"
    echo "$RAW_TX_HASH"
...
proposalChangeTx() {
    ...
    local addr=$3
    local cancel=$4
    local req='{"method":"Turingchain.CreateTransaction","params":[{"execer":"'"${EXECTOR}"'", "actionName":"PropChange", "payload":{"changes" : [{"cancel": '"${cancel}"', "addr":"'"${addr}"'"}],"startBlockHeight":'"${start}"',"endBlockHeight":'"${end}"'}}]}'
    echo "${req}"
    turingchain_Http "$req" ${HTTP} '(.error|not) and (.result != null)' "$FUNCNAME" ".result"
    turingchain_SignAndSendTx "${RETURN_RESP}" "${propKey}" "${HTTP}"
...
voteChangeTx() {
    local ID=$1
    local privk=$2
    local req='{"method":"Turingchain.CreateTransaction","params":[{"execer":"'"${EXECTOR}"'", "actionName":"VotePropChange", "payload":{"proposalID": "'"${ID}"'","approve": true}}]}'
    echo "${req}"
    turingchain_Http "$req" ${HTTP} '(.error|not) and (.result != null)' "$FUNCNAME" ".result"
    turingchain_SignAndSendTx "${RETURN_RESP}" "${privk}" "${HTTP}"
    echo "$RAW_TX_HASH"
...
testProposalTerminate() {
    ...
    turingchain_BlockWait 940 "$HTTP"
...
init() {
    ...
    if [ "$ispara" == true ]; then
        EXECTOR_ADDR=$(curl -ksd '{"method":"Turingchain.ConvertExectoAddr","params":[{"execname":"user.p.para.autonomy"}]}' ${HTTP} | jq -r ".result")
        EXECTOR="user.p.para.autonomy"
        TICKET_ADDR=$(curl -ksd '{"method":"Turingchain.ConvertExectoAddr","params":[{"execname":"user.p.para.ticket"}]}' ${HTTP} | jq -r ".result")
        TICKET_EXECTOR="user.p.para.ticket"
    else
        EXECTOR_ADDR=$(curl -ksd '{"method":"Turingchain.ConvertExectoAddr","params":[{"execname":"autonomy"}]}' ${HTTP} | jq -r ".result")
        EXECTOR="autonomy"
        TICKET_ADDR=$(curl -ksd '{"method":"Turingchain.ConvertExectoAddr","params":[{"execname":"ticket"}]}' ${HTTP} | jq -r ".result")
        TICKET_EXECTOR="ticket"
    fi
    echo "EXECTOR_ADDR=$EXECTOR_ADDR"

    local main_ip=${HTTP//8901/9671}
    turingchain_ImportPrivkey "${propKey}" "${propAddr}" "prop" "${main_ip}"

    if [ "$ispara" == false ]; then
        turingchain_applyCoinsNOLimit "$propAddr" 100000000000 "${main_ip}"
        turingchain_QueryBalance "${propAddr}" "$main_ip"

    else
        turingchain_applyCoins "$propAddr" 1000000000 "${main_ip}"
        turingchain_QueryBalance "${propAddr}" "$main_ip"
        #主链投票账户转帐
        handleBoards "$main_ip"

        local para_ip="${HTTP}"
        turingchain_ImportPrivkey "${propKey}" "${propAddr}" "prop" "$para_ip"

        #平行链中账户转帐
        turingchain_applyCoinsNOLimit "$propAddr" 100000000000 "$para_ip"
        turingchain_QueryBalance "$propAddr" "$para_ip"
        turingchain_para_init "$para_ip"
    fi

    # 往合约中转
    turingchain_SendToAddress "$propAddr" "$EXECTOR_ADDR" 90000000000 "$HTTP"
    turingchain_QueryExecBalance "$propAddr" "$EXECTOR" "$HTTP"

    # 往ticket合约中转帐
    turingchain_SendToAddress "$voteAddr" "$TICKET_ADDR" 300100000000 "$HTTP"
    turingchain_QueryExecBalance "$voteAddr" "$TICKET_EXECTOR" "$HTTP"
    # 往投票账户中转帐
    handleBoards "$HTTP"
}

function rpc_test() {
    turingchain_RpcTestBegin autonomy
    ...
    turingchain_RpcTestRst autonomy "$CASE_ERR"
}
