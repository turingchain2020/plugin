...
contract Oracle {
    ...
    */
    ...
    {
        require(
            turingchainBridge.isProphecyClaimActive(
                _claimID
            ) == true,
    ...
    */
    ...
    {
        require(
           turingchainBridge.isValidClaimType(
               uint8(_claimType)
           ) == true,
    ...
    */
    ...
    {
        operator = _operator;
        turingchainBridge = TuringchainBridge(_turingchainBridge);
        valset = Valset(_valset);
    }

    /*
    * @dev: newOracleClaim
    *       Allows validators to make new OracleClaims on turingchain lock/burn prophecy,
    *       if the required vote power reached,just make it processed
    * @param _claimType: burn or lock,
    * @param _turingchainSender: turingchain sender,
    ...
    */
        ...
        isValidClaimType(_claimType)
    {
        ...
        if (oracleClaimValidators[_claimID].length == 0) {
             turingchainBridge.setNewProphecyClaim(
                            _claimID,
                            uint8(_claimType),
                            _turingchainSender,
                            _ethereumReceiver,
        ...
        if (true == valid)  {
            ...
            if (turingchainBridge.isProphecyClaimActive(_claimID) == true) {
    ...
    */
    function completeClaim(
        ...
        internal
    {
        turingchainBridge.completeClaim(
            _claimID
        );
    }
}