...
var (
	turingchainAddr  = "14KEKbYtKKQm4wMthSK9J4La4nAiidGozt"
...
func (c *suiteContracts) Test_IsProphecyPending() {
	...
	bret, err := IsProphecyPending(claimID, c.para.InitValidators[0], c.x2EthContracts.TuringchainBridge)
	require.Nil(c.T(), err)
	assert.Equal(c.T(), bret, false)
}

func (c *suiteContracts) Test_LogLockToEthBridgeClaim() {
	to := common.FromHex(turingchainAddr)
	...
	assert.Equal(c.T(), witnessClaim.TuringchainReceiver, string(event.To))
...
func (c *suiteContracts) Test_LogBurnToEthBridgeClaim() {
	...
	assert.Equal(c.T(), witnessClaim.TuringchainReceiver, string(event.TuringchainReceiver))
	assert.Equal(c.T(), witnessClaim.Amount, "100")
	assert.Equal(c.T(), witnessClaim.Nonce, event.Nonce.Int64())
	assert.Equal(c.T(), witnessClaim.Decimal, int64(8))
}

func (c *suiteContracts) Test_ParseBurnLockTxReceipt_TuringchainMsgToProphecyClaim() {
	claimType := events.MsgBurn
	turingchainToEth := types.ReceiptTuringchainToEth{
		TuringchainSender:    turingchainAddr,
	...
	log := &turingchainTypes.ReceiptLog{
		Ty:  types.TyWithdrawTuringchainLog,
		Log: turingchainTypes.Encode(&turingchainToEth),
	}

	var logs []*turingchainTypes.ReceiptLog
	logs = append(logs, log)

	receipt := &turingchainTypes.ReceiptData{
		Ty:   types.TyWithdrawTuringchainLog,
		Logs: logs,
	}

	turingchainMsg := ParseBurnLockTxReceipt(claimType, receipt)
	require.NotNil(c.T(), turingchainMsg)
	assert.Equal(c.T(), turingchainMsg.ClaimType, claimType)
	assert.Equal(c.T(), turingchainMsg.TuringchainSender, []byte(turingchainToEth.TuringchainSender))
	assert.Equal(c.T(), turingchainMsg.EthereumReceiver, common.HexToAddress(turingchainToEth.EthereumReceiver))
	assert.Equal(c.T(), turingchainMsg.TokenContractAddress, common.HexToAddress(turingchainToEth.TokenContract))
	assert.Equal(c.T(), turingchainMsg.Symbol, turingchainToEth.IssuerDotSymbol)
	assert.Equal(c.T(), turingchainMsg.Amount.String(), "100")

	prophecyClaim := TuringchainMsgToProphecyClaim(*turingchainMsg)
	assert.Equal(c.T(), turingchainMsg.ClaimType, prophecyClaim.ClaimType)
	assert.Equal(c.T(), turingchainMsg.TuringchainSender, prophecyClaim.TuringchainSender)
	assert.Equal(c.T(), turingchainMsg.EthereumReceiver, prophecyClaim.EthereumReceiver)
	assert.Equal(c.T(), turingchainMsg.TokenContractAddress, prophecyClaim.TokenContractAddress)
	assert.Equal(c.T(), strings.ToLower(turingchainMsg.Symbol), prophecyClaim.Symbol)
	assert.Equal(c.T(), turingchainMsg.Amount, prophecyClaim.Amount)
}
...
func (c *suiteContracts) Test_CreateBridgeToken() {
	...
	tokenAddr, err := CreateBridgeToken("trc", c.sim, operatorInfo, c.x2EthDeployInfo, c.x2EthContracts)
	require.Nil(c.T(), err)
	c.sim.Commit()

	addr, err := GetToken2address(c.x2EthContracts.BridgeBank, "trc")
	require.Nil(c.T(), err)
	assert.Equal(c.T(), addr, tokenAddr)

	turingchainSender := []byte("14KEKbYtKKQm4wMthSK9J4La4nAiidGozt")
	amount := int64(100)
	ethReceiver := c.para.InitValidators[2]
	claimID := crypto.Keccak256Hash(turingchainSender, ethReceiver.Bytes(), big.NewInt(amount).Bytes())
	...
	turingchainReceiver := "1GTxrmuWiXavhcvsaH5w9whgVxUrWsUMdV"
	{
		amount := "10"
		bn := big.NewInt(1)
		bn, _ = bn.SetString(x2ethTypes.TrimZeroAndDot(amount), 10)
		txhash, err := Burn(hexutil.Encode(crypto.FromECDSA(c.para.ValidatorPriKey[2])), tokenAddr, turingchainReceiver, c.x2EthDeployInfo.BridgeBank.Address, bn, c.x2EthContracts.BridgeBank, c.sim)
...

	{
		...
		_, err = BurnAsync(hexutil.Encode(crypto.FromECDSA(c.para.ValidatorPriKey[2])), tokenAddr, turingchainReceiver, bn, c.x2EthContracts.BridgeBank, c.sim)
		require.Nil(c.T(), err)
		c.sim.Commit()
...
func (c *suiteContracts) Test_CreateERC20Token() {
...

	{
		...
		bn := big.NewInt(1)
		bn, _ = bn.SetString(x2ethTypes.TrimZeroAndDot(amount), 10)
		turingchainReceiver := "14KEKbYtKKQm4wMthSK9J4La4nAiidGozt"
		_, err = LockEthErc20Asset(hexutil.Encode(crypto.FromECDSA(c.para.DeployPrivateKey)), tokenAddr, turingchainReceiver, bn, c.sim, c.x2EthContracts.BridgeBank, c.x2EthDeployInfo.BridgeBank.Address)
		require.Nil(c.T(), err)
		c.sim.Commit()
...

	{
		...
		turingchainReceiver := "14KEKbYtKKQm4wMthSK9J4La4nAiidGozt"
		_, err = LockEthErc20AssetAsync(hexutil.Encode(crypto.FromECDSA(c.para.DeployPrivateKey)), tokenAddr, turingchainReceiver, bn, c.sim, c.x2EthContracts.BridgeBank)
		require.Nil(c.T(), err)
		c.sim.Commit()
