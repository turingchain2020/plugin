...
//IsProphecyPending ...
func IsProphecyPending(claimID [32]byte, validator common.Address, turingchainBridge *generated.TuringchainBridge) (bool, error) {
	opts := &bind.CallOpts{
		Pending: true,
		From:    validator,
		Context: context.Background(),
	...
	// Initialize BridgeRegistry instance
	active, err := turingchainBridge.IsProphecyClaimActive(opts, claimID)
	if err != nil {
		txslog.Error("IsActiveValidatorFromTuringchainBridge", "Failed to query IsActiveValidator due to:", err.Error())
		return false, err
	}
	return active, nil
}
