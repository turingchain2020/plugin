...
import (
	...
	"github.com/turingchain2020/plugin/plugin/dapp/x2ethereum/ebrelayer/relayer/turingchain"
	"github.com/turingchain2020/plugin/plugin/dapp/x2ethereum/ebrelayer/relayer/ethereum"
...
//Manager ...
type Manager struct {
	turingchainRelayer *turingchain.Relayer4Turingchain
...
//NewRelayerManager ...
//1.验证人的私钥需要通过cli命令行进行导入，且turingchain和ethereum两种不同的验证人需要分别导入
...
func NewRelayerManager(turingchainRelayer *turingchain.Relayer4Turingchain, ethRelayer *ethereum.Relayer4Ethereum, db dbm.DB) *Manager {
	l, _ := lru.New(4096)
	manager := &Manager{
		turingchainRelayer: turingchainRelayer,
...
func (manager *Manager) SetPassphase(setPasswdReq relayerTypes.ReqSetPasswd, result *interface{}) error {
	...
	if !utils.IsValidPassWord(setPasswdReq.Passphase) {
		return turingchainTypes.ErrInvalidPassWord
	}
...
func (manager *Manager) ChangePassphase(setPasswdReq relayerTypes.ReqChangePasswd, result *interface{}) error {
	...
	if len(manager.passphase) == 0 && manager.encryptFlag == EncryptEnable {
		...
		if !isok {
			...
			return turingchainTypes.ErrVerifyOldpasswdFail
		}
	}
	...
	err = manager.turingchainRelayer.StoreAccountWithNewPassphase(setPasswdReq.NewPassphase, setPasswdReq.OldPassphase)
	if err != nil {
		mlog.Error("ChangePassphase", "StoreAccountWithNewPassphase err", err)
		return err
	}
...
func (manager *Manager) Unlock(passphase string, result *interface{}) error {
	...
	if err := manager.turingchainRelayer.RestorePrivateKeys(passphase); nil != err {
		info := fmt.Sprintf("Failed to RestorePrivateKeys for turingchainRelayer due to:%s", err.Error())
...
func (manager *Manager) ImportTuringchainRelayerPrivateKey(importKeyReq relayerTypes.ImportKeyReq, result *interface{}) error {
	...
	_, err := manager.turingchainRelayer.ImportPrivateKey(manager.passphase, privateKey)
	if err != nil {
		mlog.Error("ImportTuringchainValidatorPrivateKey", "Failed due to cause:", err.Error())
		return err
	}

	*result = rpctypes.Reply{
		IsOk: true,
		Msg:  "Succeed to import private key for turingchain relayer",
	}
	return nil
}
...
func (manager *Manager) ImportTuringchainPrivateKey4EthRelayer(privateKey string, result *interface{}) error {
	...
	if err := manager.ethRelayer.ImportTuringchainPrivateKey(manager.passphase, privateKey); nil != err {
		return err
	}
	*result = rpctypes.Reply{
		IsOk: true,
		Msg:  "Succeed to import turingchain private key for ethereum relayer",
	}
	return nil
}
...
func (manager *Manager) ShowTuringchainRelayerValidator(param interface{}, result *interface{}) error {
	...
	defer manager.mtx.Unlock()
	var err error
	*result, err = manager.turingchainRelayer.GetAccountAddr()
	if nil != err {
		return err
	}
...
func (manager *Manager) Burn(burn relayerTypes.Burn, result *interface{}) error {
	...
	txhash, err := manager.ethRelayer.Burn(burn.OwnerKey, burn.TokenAddr, burn.TuringchainReceiver, burn.Amount)
	if nil != err {
		return err
	}
	*result = rpctypes.Reply{
...
func (manager *Manager) BurnAsync(burn relayerTypes.Burn, result *interface{}) error {
	...
	txhash, err := manager.ethRelayer.BurnAsync(burn.OwnerKey, burn.TokenAddr, burn.TuringchainReceiver, burn.Amount)
	if nil != err {
		return err
	}
	*result = rpctypes.Reply{
...
func (manager *Manager) LockEthErc20AssetAsync(lockEthErc20Asset relayerTypes.LockEthErc20, result *interface{}) error {
	...
	txhash, err := manager.ethRelayer.LockEthErc20AssetAsync(lockEthErc20Asset.OwnerKey, lockEthErc20Asset.TokenAddr, lockEthErc20Asset.Amount, lockEthErc20Asset.TuringchainReceiver)
	if nil != err {
		return err
	}
	*result = rpctypes.Reply{
...
func (manager *Manager) LockEthErc20Asset(lockEthErc20Asset relayerTypes.LockEthErc20, result *interface{}) error {
	...
	txhash, err := manager.ethRelayer.LockEthErc20Asset(lockEthErc20Asset.OwnerKey, lockEthErc20Asset.TokenAddr, lockEthErc20Asset.Amount, lockEthErc20Asset.TuringchainReceiver)
	if nil != err {
		return err
	}
	*result = rpctypes.Reply{
