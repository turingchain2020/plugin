...
import (
	...
	l "github.com/turingchain2020/turingchain/common/log/log15"
...
//向turingchain节点的注册推送交易回执，AddSubscribeTxReceipt具有2种功能：
...
func bindOrResumePush(cfg *relayerTypes.SyncTxReceiptConfig) {
	...
	ctx := jsonclient.NewRPCCtx(cfg.TuringchainHost, "Turingchain.AddPushSubscribe", params, &res)
	_, err := ctx.RunResult()
	if err != nil {
		fmt.Println("Failed to AddSubscribeTxReceipt to  rpc addr:", cfg.TuringchainHost, "ReplySubTxReceipt", res)
	...
	log.Info("bindOrResumePush", "Succeed to AddSubscribeTxReceipt for rpc address:", cfg.TuringchainHost)
