...
func test_HandleRequest(t *testing.T) {
	...
	turingchainRelayer := newTuringchainRelayer(sim, x2EthDeployInfo, "127.0.0.1:60002", t)
	_, err = turingchainRelayer.ImportPrivateKey(passphrase, privateKeyStr)
	assert.NoError(t, err)

	body, err := hex.DecodeString(test)
	assert.NoError(t, err)

	turingchainRelayer.statusCheckedIndex = 1220
	err = syncTx.HandleRequest(body)
	assert.NoError(t, err)

	time.Sleep(200 * time.Millisecond)

	ret := turingchainRelayer.QueryTxhashRelay2Eth()
...
func test_ImportPrivateKey(t *testing.T) {
	...
	require.NoError(t, err)
	turingchainRelayer := newTuringchainRelayer(sim, x2EthDeployInfo, "127.0.0.1:60000", t)

	addr, err := turingchainRelayer.ImportPrivateKey(passphrase, privateKeyStr)
	assert.NoError(t, err)
	assert.Equal(t, addr, accountAddr)

	time.Sleep(50 * time.Millisecond)

	addr, err = turingchainRelayer.GetAccountAddr()
	assert.NoError(t, err)
	assert.Equal(t, addr, accountAddr)

	key, _, _ := turingchainRelayer.GetAccount("123")
	assert.NotEqual(t, key, privateKeyStr)

	key, _, _ = turingchainRelayer.GetAccount(passphrase)
	assert.Equal(t, key, privateKeyStr)
}

func test_Locktrc(t *testing.T) {
	para, sim, x2EthContracts, x2EthDeployInfo, err := setup.DeployContracts()
	require.NoError(t, err)
	turingchainRelayer := newTuringchainRelayer(sim, x2EthDeployInfo, "127.0.0.1:60001", t)
	_, err = turingchainRelayer.ImportPrivateKey(passphrase, privateKeyStr)
	assert.NoError(t, err)
	...
	claimID := crypto.Keccak256Hash(turingchainSender, ethReceiver.Bytes(), logEvent.Token.Bytes(), big.NewInt(amount).Bytes())
	...
	turingchainRelayer.statusCheckedIndex = 1
	turingchainRelayer.totalTx4TuringchainToEth = 2
	turingchainRelayer.rwLock.Unlock()
	_ = turingchainRelayer.setLastestRelay2EthTxhash(relayerTx.EthTxPending.String(), txhash, 2)

	time.Sleep(200 * time.Millisecond)

	turingchainRelayer.rwLock.Lock()
	turingchainRelayer.statusCheckedIndex = 9
	turingchainRelayer.totalTx4TuringchainToEth = 11
	turingchainRelayer.rwLock.Unlock()
	_ = turingchainRelayer.setLastestRelay2EthTxhash(relayerTx.EthTxPending.String(), "", 11)
...
func test_restorePrivateKeys(t *testing.T) {
	_, sim, _, x2EthDeployInfo, err := setup.DeployContracts()
	require.NoError(t, err)
	turingchainRelayer := newTuringchainRelayer(sim, x2EthDeployInfo, "127.0.0.1:60003", t)
	_, err = turingchainRelayer.ImportPrivateKey(passphrase, privateKeyStr)
	assert.NoError(t, err)

	go func() {
		for range turingchainRelayer.unlock {
		}
	}()
	temp := turingchainRelayer.ethSender

	err = turingchainRelayer.RestorePrivateKeys("123")
	assert.NotEqual(t, hex.EncodeToString(temp.Bytes()), hex.EncodeToString(turingchainRelayer.ethSender.Bytes()))
	assert.NoError(t, err)

	err = turingchainRelayer.RestorePrivateKeys(passphrase)
	assert.Equal(t, hex.EncodeToString(temp.Bytes()), hex.EncodeToString(turingchainRelayer.ethSender.Bytes()))
	assert.NoError(t, err)

	err = turingchainRelayer.StoreAccountWithNewPassphase("new123", passphrase)
	assert.NoError(t, err)

	err = turingchainRelayer.RestorePrivateKeys("new123")
	assert.Equal(t, hex.EncodeToString(temp.Bytes()), hex.EncodeToString(turingchainRelayer.ethSender.Bytes()))
	assert.NoError(t, err)
...
func newTuringchainRelayer(sim *ethinterface.SimExtend, x2EthDeployInfo *ethtxs.X2EthDeployInfo, pushBind string, t *testing.T) *Relayer4Turingchain {
	...
	relayer := &Relayer4Turingchain{
		rpcLaddr:            cfg.SyncTxConfig.TuringchainHost,
	...
	relayer.totalTx4TuringchainToEth = relayer.getTotalTxAmount2Eth()
	...
	syncCfg := &ebTypes.SyncTxReceiptConfig{
		TuringchainHost:       cfg.SyncTxConfig.TuringchainHost,
...
func newMock33() *testnode.TuringchainMock {
