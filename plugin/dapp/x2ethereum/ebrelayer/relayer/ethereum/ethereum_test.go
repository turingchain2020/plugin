...
import (
	...
	"github.com/turingchain2020/plugin/plugin/dapp/x2ethereum/ebrelayer/ethcontract/test/setup"
	...
	relayerTypes "github.com/turingchain2020/plugin/plugin/dapp/x2ethereum/ebrelayer/types"
...
var (
	...
	turingchainAccountAddr   = "1GTxrmuWiXavhcvsaH5w9whgVxUrWsUMdV"
...
func Test_GetValidatorAddr(t *testing.T) {
	...
	_ = ethRelayer.ImportTuringchainPrivateKey(passphrase, turingchainPrivateKeyStr)
	...
	require.Nil(t, err)
	assert.NotEqual(t, privateKey, turingchainPrivateKeyStr)

	privateKey, addr, err := ethRelayer.GetAccount(passphrase)
	require.Nil(t, err)
	assert.Equal(t, privateKey, turingchainPrivateKeyStr)
	assert.Equal(t, addr, turingchainAccountAddr)

	validators, err := ethRelayer.GetValidatorAddr()
	require.Nil(t, err)
	assert.Equal(t, validators.TuringchainValidator, turingchainAccountAddr)
}
...
func Test_ShowAddr(t *testing.T) {
	{
		...
		relayer := &Relayer4Ethereum{
			...
			rpcURL2Turingchain:      cfg.SyncTxConfig.TuringchainHost,
...
func Test_CreateBridgeToken(t *testing.T) {
	...
	tokenAddrtrc, err := ethRelayer.CreateBridgeToken("TRC")
	require.Nil(t, err)
	require.NotEmpty(t, tokenAddrtrc)
	sim.Commit()

	addr, err := ethRelayer.ShowTokenAddrBySymbol("TRC")
	require.Nil(t, err)
	assert.Equal(t, addr, tokenAddrtrc)

	decimals, err := ethRelayer.GetDecimals(tokenAddrtrc)
	require.Nil(t, err)
	assert.Equal(t, decimals, uint8(8))

	_, err = ethRelayer.Burn(para.InitValidators[0].String(), tokenAddrtrc, turingchainAccountAddr, "10")
	require.Error(t, err)

	_, err = ethRelayer.BurnAsync(para.InitValidators[0].String(), tokenAddrtrc, turingchainAccountAddr, "10")
	require.Error(t, err)
}
...
func testLockEth(t *testing.T) {
	...
	_, err = x2EthContracts.BridgeBank.Lock(userOneAuth, turingchainSender, common.Address{}, ethAmount)
	require.Nil(t, err)
	sim.Commit()
...
func testCreateERC20Token(t *testing.T) {
	...
	tx2 := ethRelayer.QueryTxhashRelay2Turingchain()
	require.Empty(t, tx2)
	...
	_, err = ethRelayer.LockEthErc20Asset(hexutil.Encode(crypto.FromECDSA(para.DeployPrivateKey)), tokenErc20Addrtestc, "100", turingchainReceiver)
	require.Nil(t, err)
	sim.Commit()
	...
	_, err = ethRelayer.LockEthErc20AssetAsync(hexutil.Encode(crypto.FromECDSA(para.DeployPrivateKey)), tokenErc20Addrtestc, "100", turingchainReceiver)
	require.Nil(t, err)
	sim.Commit()
...
func testBurnTrc(t *testing.T) {
	...
	_, err = x2EthContracts.Oracle.NewOracleClaim(
		...
		turingchainSender,
		ethReceiver,
		common.HexToAddress(tokenAddrtrc),
		"trc",
		big.NewInt(amount),
		claimID,
		signature)
	...
	balanceNew, err := ethRelayer.GetBalance(tokenAddrtrc, ethReceiver.String())
	require.Nil(t, err)
	require.Equal(t, balanceNew, "100")

	_, err = ethRelayer.Burn(hexutil.Encode(crypto.FromECDSA(para.ValidatorPriKey[2])), tokenAddrtrc, turingchainAccountAddr, "10")
	require.NoError(t, err)
	sim.Commit()
	...
	_, err = ethRelayer.ApproveAllowance(hexutil.Encode(crypto.FromECDSA(para.ValidatorPriKey[2])), tokenAddrtrc, "10")
	require.Nil(t, err)
	sim.Commit()

	_, err = ethRelayer.BurnAsync(hexutil.Encode(crypto.FromECDSA(para.ValidatorPriKey[2])), tokenAddrtrc, turingchainAccountAddr, "10")
	require.NoError(t, err)
	sim.Commit()
...
func Test_RestorePrivateKeys(t *testing.T) {
	...
	temp := ethRelayer.privateKey4Turingchain
	ethRelayer.rwLock.RUnlock()

	err = ethRelayer.RestorePrivateKeys("123")
	ethRelayer.rwLock.RLock()
	assert.NotEqual(t, hex.EncodeToString(temp.Bytes()), hex.EncodeToString(ethRelayer.privateKey4Turingchain.Bytes()))
	ethRelayer.rwLock.RUnlock()
	require.Nil(t, err)

	err = ethRelayer.RestorePrivateKeys(passphrase)
	ethRelayer.rwLock.RLock()
	assert.Equal(t, hex.EncodeToString(temp.Bytes()), hex.EncodeToString(ethRelayer.privateKey4Turingchain.Bytes()))
	ethRelayer.rwLock.RUnlock()
	require.Nil(t, err)
...
func newEthRelayer(para *ethtxs.DeployPara, sim *ethinterface.SimExtend, x2EthContracts *ethtxs.X2EthContracts, x2EthDeployInfo *ethtxs.X2EthDeployInfo) *Relayer4Ethereum {
	cfg := initCfg(*configPath)
	cfg.SyncTxConfig.TuringchainHost = "http://127.0.0.1:9671"
