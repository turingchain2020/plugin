...
import (
	...
	"github.com/turingchain2020/plugin/plugin/dapp/x2ethereum/ebrelayer/ethtxs"
	...
	x2ethTypes "github.com/turingchain2020/plugin/plugin/dapp/x2ethereum/types"
...
type Relayer4Ethereum struct {
	...
	db                     dbm.DB
	rwLock                 sync.RWMutex
	privateKey4Turingchain     turingchainCrypto.PrivKey
	ethValidator           common.Address
	totalTx4Eth2Turingchain    int64
	rpcURL2Turingchain         string
	unlockchan             chan int
...
func StartEthereumRelayer(rpcURL2Turingchain string, db dbm.DB, provider, registryAddress string, deploy *ebTypes.Deploy, degree, blockInterval int32) *Relayer4Ethereum {
	if 0 == blockInterval {
	...
	ethRelayer := &Relayer4Ethereum{
		...
		rpcURL2Turingchain:      rpcURL2Turingchain,
	...
	relayerLog.Info("Relayer4Ethereum proc", "Started Ethereum websocket with provider:", ethRelayer.provider, "rpcURL2Turingchain:", ethRelayer.rpcURL2Turingchain)
...
func (ethRelayer *Relayer4Ethereum) IsProphecyPending(claimID [32]byte) (bool, error) {
	return ethtxs.IsProphecyPending(claimID, ethRelayer.ethValidator, ethRelayer.x2EthContracts.TuringchainBridge)
}
...
//Burn ...
func (ethRelayer *Relayer4Ethereum) Burn(ownerPrivateKey, tokenAddr, turingchainReceiver, amount string) (string, error) {
	bn := big.NewInt(1)
	bn, _ = bn.SetString(x2ethTypes.TrimZeroAndDot(amount), 10)
	return ethtxs.Burn(ownerPrivateKey, tokenAddr, turingchainReceiver, ethRelayer.x2EthDeployInfo.BridgeBank.Address, bn, ethRelayer.x2EthContracts.BridgeBank, ethRelayer.clientSpec)
}

//BurnAsync ...
func (ethRelayer *Relayer4Ethereum) BurnAsync(ownerPrivateKey, tokenAddr, turingchainReceiver, amount string) (string, error) {
	bn := big.NewInt(1)
	bn, _ = bn.SetString(x2ethTypes.TrimZeroAndDot(amount), 10)
	return ethtxs.BurnAsync(ownerPrivateKey, tokenAddr, turingchainReceiver, bn, ethRelayer.x2EthContracts.BridgeBank, ethRelayer.clientSpec)
}
...
//LockEthErc20Asset ...
func (ethRelayer *Relayer4Ethereum) LockEthErc20Asset(ownerPrivateKey, tokenAddr, amount string, turingchainReceiver string) (string, error) {
	bn := big.NewInt(1)
	bn, _ = bn.SetString(x2ethTypes.TrimZeroAndDot(amount), 10)
	return ethtxs.LockEthErc20Asset(ownerPrivateKey, tokenAddr, turingchainReceiver, bn, ethRelayer.clientSpec, ethRelayer.x2EthContracts.BridgeBank, ethRelayer.x2EthDeployInfo.BridgeBank.Address)
}

//LockEthErc20AssetAsync ...
func (ethRelayer *Relayer4Ethereum) LockEthErc20AssetAsync(ownerPrivateKey, tokenAddr, amount string, turingchainReceiver string) (string, error) {
	bn := big.NewInt(1)
	bn, _ = bn.SetString(x2ethTypes.TrimZeroAndDot(amount), 10)
	return ethtxs.LockEthErc20AssetAsync(ownerPrivateKey, tokenAddr, turingchainReceiver, bn, ethRelayer.clientSpec, ethRelayer.x2EthContracts.BridgeBank)
}
...
func (ethRelayer *Relayer4Ethereum) proc() {
	...
	for range ethRelayer.unlockchan {
		...
		ethRelayer.rwLock.RLock()
		privateKey4Turingchain := ethRelayer.privateKey4Turingchain
		ethRelayer.rwLock.RUnlock()
		if nil != privateKey4Turingchain && nilAddr != ethRelayer.bridgeRegistryAddr {
...
func (ethRelayer *Relayer4Ethereum) storeBridgeBankLogs(vLog types.Log, setBlockNumber bool) {
	//lock,用于捕捉 (ETH/ERC20----->turingchain) 跨链转移
	//burn,用于捕捉 (turingchain token----->turingchain) 实现turingchain资产withdraw操作，之后在turingchain上实现unlock操作
...
func (ethRelayer *Relayer4Ethereum) procBridgeBankLogs(vLog types.Log) {
	...
	if vLog.Topics[0].Hex() == ethRelayer.bridgeBankEventLockSig {
		...
		//burn,用于捕捉 (turingchain token----->turingchain) 实现turingchain资产withdraw操作，之后在turingchain上实现unlock操作
		eventName := events.LogTuringchainTokenBurn.String()
		relayerLog.Info("Relayer4Ethereum proc", "Going to process", eventName,
			"Block number:", vLog.BlockNumber, "Tx hash:", vLog.TxHash.Hex())
...
func (ethRelayer *Relayer4Ethereum) QueryTxhashRelay2Eth() ebTypes.Txhashes {
	txhashs := ethRelayer.queryTxhashes([]byte(turingchainToEthTxHashPrefix))
	return ebTypes.Txhashes{Txhash: txhashs}
}

//QueryTxhashRelay2Turingchain ...
func (ethRelayer *Relayer4Ethereum) QueryTxhashRelay2Turingchain() ebTypes.Txhashes {
	txhashs := ethRelayer.queryTxhashes([]byte(eth2turingchainTxHashPrefix))
	return ebTypes.Txhashes{Txhash: txhashs}
}

// handleLogLockEvent : unpacks a LogLock event, converts it to a ProphecyClaim, and relays a tx to turingchain
func (ethRelayer *Relayer4Ethereum) handleLogLockEvent(clientChainID *big.Int, contractABI abi.ABI, eventName string, log types.Log) error {
	rpcURL := ethRelayer.rpcURL2Turingchain
	...
	privateKey4Turingchain := ethRelayer.privateKey4Turingchain
	ethRelayer.rwLock.RUnlock()
	txhash, err := ethtxs.RelayLockToTuringchain(privateKey4Turingchain, prophecyClaim, rpcURL)
	if err != nil {
		relayerLog.Error("handleLogLockEvent", "Failed to RelayLockToTuringchain due to:", err.Error())
		return err
	}
	relayerLog.Info("handleLogLockEvent", "RelayLockToTuringchain with hash", txhash)

	//保存交易hash，方便查询
	atomic.AddInt64(&ethRelayer.totalTx4Eth2Turingchain, 1)
	txIndex := atomic.LoadInt64(&ethRelayer.totalTx4Eth2Turingchain)
	if err = ethRelayer.updateTotalTxAmount2turingchain(txIndex); nil != err {
		relayerLog.Error("handleLogLockEvent", "Failed to RelayLockToTuringchain due to:", err.Error())
		return err
	}
	if err = ethRelayer.setLastestRelay2TuringchainTxhash(txhash, txIndex); nil != err {
		relayerLog.Error("handleLogLockEvent", "Failed to RelayLockToTuringchain due to:", err.Error())
		return err
	}
...
// handleLogBurnEvent : unpacks a burn event, converts it to a ProphecyClaim, and relays a tx to turingchain
func (ethRelayer *Relayer4Ethereum) handleLogBurnEvent(clientChainID *big.Int, contractABI abi.ABI, eventName string, log types.Log) error {
	...
	privateKey4Turingchain := ethRelayer.privateKey4Turingchain
	ethRelayer.rwLock.RUnlock()
	txhash, err := ethtxs.RelayBurnToTuringchain(privateKey4Turingchain, prophecyClaim, rpcURL)
	if err != nil {
		relayerLog.Error("handleLogLockEvent", "Failed to RelayLockToTuringchain due to:", err.Error())
		return err
	}
	relayerLog.Info("handleLogLockEvent", "RelayBurnToTuringchain with hash", txhash)

	//保存交易hash，方便查询
	atomic.AddInt64(&ethRelayer.totalTx4Eth2Turingchain, 1)
	txIndex := atomic.LoadInt64(&ethRelayer.totalTx4Eth2Turingchain)
	if err = ethRelayer.updateTotalTxAmount2turingchain(txIndex); nil != err {
		relayerLog.Error("handleLogBurnEvent", "Failed to RelayLockToTuringchain due to:", err.Error())
		return err
	}
	if err = ethRelayer.setLastestRelay2TuringchainTxhash(txhash, txIndex); nil != err {
		relayerLog.Error("handleLogBurnEvent", "Failed to RelayLockToTuringchain due to:", err.Error())
