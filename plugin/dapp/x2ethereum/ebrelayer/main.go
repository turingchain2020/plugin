...
import (
	...
	logf "github.com/turingchain2020/turingchain/common/log"
	"github.com/turingchain2020/turingchain/common/log/log15"
	turingchainTypes "github.com/turingchain2020/turingchain/types"
	"github.com/turingchain2020/plugin/plugin/dapp/x2ethereum/ebrelayer/relayer"
	turingchainRelayer "github.com/turingchain2020/plugin/plugin/dapp/x2ethereum/ebrelayer/relayer/turingchain"
	ethRelayer "github.com/turingchain2020/plugin/plugin/dapp/x2ethereum/ebrelayer/relayer/ethereum"
...
func main() {
	...
	log.Info("Starting FUZAMEI Turingchain-X-Ethereum relayer software:", "\n     Name: ", cfg.Title)
	...
	turingchainRelayerService := turingchainRelayer.StartTuringchainRelayer(ctx, cfg.SyncTxConfig, cfg.BridgeRegistry, cfg.EthProvider, db)
	ethRelayerService := ethRelayer.StartEthereumRelayer(cfg.SyncTxConfig.TuringchainHost, db, cfg.EthProvider, cfg.BridgeRegistry, cfg.Deploy, cfg.EthMaturityDegree, cfg.EthBlockFetchPeriod)

	relayerManager := relayer.NewRelayerManager(turingchainRelayerService, ethRelayerService, db)
...
func convertLogCfg(log *relayerTypes.Log) *turingchainTypes.Log {
	return &turingchainTypes.Log{
