...
privateKeys[6]="9dc6df3a8ab139a54d8a984f54958ae0661f967229bf3bdbb886b87d58b56a08"
...
loop_send_lock_eth() {
    ...
    preTuringchainBalance=$(${TuringchainCli} x2ethereum balance -s 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv -t eth | jq ".res" | jq ".[]" | jq ".balance" | sed 's/\"//g')
    ...
    nowTuringchainBalance=$(${TuringchainCli} x2ethereum balance -s 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv -t eth | jq ".res" | jq ".[]" | jq ".balance" | sed 's/\"//g')
    diff=$(echo "$nowTuringchainBalance - $preTuringchainBalance" | bc)
    check_number "${diff}" 7
}

loop_send_burn_eth() {
    echo -e "${GRE}=========== Turingchain Burn begin ===========${NOC}"
    ...
    while [[ i -lt ${#privateKeys[@]} ]]; do
        preEthBalance[$i]=$(curl -ksd '{"jsonrpc":"2.0","method":"eth_getBalance","params":["'${ethAddress[i]}'", "latest"],"id":1}' "${ethUrl}" | jq -r ".result")
        ethTxHash=$(${TuringchainCli} send x2ethereum burn -a 1 -r ${ethAddress[i]} -t eth --node_addr "${ethUrl}" -k 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv)
        echo ${i} "burn turingchain tx hash:" "${ethTxHash}"
        # shellcheck disable=SC2219
        let i++
    done
    ...
    nowTuringchainBalance=$(${TuringchainCli} x2ethereum balance -s 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv -t eth | jq ".res" | jq ".[]" | jq ".balance" | sed 's/\"//g')
    diff=$(echo "$preTuringchainBalance - $nowTuringchainBalance" | bc)
    check_number "${diff}" 7
}

loop_send_lock_trc() {
    echo -e "${GRE}=========== Turingchain Lock begin ===========${NOC}"

    preTuringchainBalance=$(${TuringchainCli} account balance -a 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv -e x2ethereum | jq -r ".balance" | sed 's/\"//g')

    i=0
    while [[ i -lt ${#privateKeys[@]} ]]; do
        preEthBalance[$i]=$(${CLIA} relayer ethereum balance -o "${ethAddress[i]}" -t "${tokenAddrTrc}" | jq -r ".balance")
        ethTxHash=$(${TuringchainCli} send x2ethereum lock -q "${tokenAddrTrc}" -a 1 -r ${ethAddress[i]} -t coins.trc --node_addr "${ethUrl}" -k 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv)
        echo ${i} "lock turingchain tx hash:" "${ethTxHash}"
        # shellcheck disable=SC2219
        let i++
    done
    ...
    i=0
    while [[ i -lt ${#privateKeys[@]} ]]; do
        nowEthBalance=$(${CLIA} relayer ethereum balance -o "${ethAddress[i]}" -t "${tokenAddrTrc}" | jq -r ".balance")
        res=$((nowEthBalance - preEthBalance[i]))
        echo ${i} "preBalance" "${preEthBalance[i]}" "nowBalance" "${nowEthBalance}" "diff" ${res}
        check_number "${res}" 1
        # shellcheck disable=SC2219
    ...
    nowTuringchainBalance=$(${TuringchainCli} account balance -a 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv -e x2ethereum | jq -r ".balance" | sed 's/\"//g')
    diff=$(echo "$preTuringchainBalance - $nowTuringchainBalance" | bc)
    check_number "${diff}" 7
}
...
loop_send_burn_trc() {
    ...
    while [[ i -lt ${#privateKeys[@]} ]]; do
        preEthBalance[$i]=$(${CLIA} relayer ethereum balance -o "${ethAddress[i]}" -t "${tokenAddrTrc}" | jq -r ".balance")
        approveTxHash=$(${CLIA} relayer ethereum approve -m 1 -k "${privateKeys[i]}" -t "${tokenAddrTrc}")
        ethTxHash=$(${CLIA} relayer ethereum burn-async -m 1 -k "${privateKeys[i]}" -r 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv -t "${tokenAddrTrc}")
...
loop_send_lock_erc20() {
    ...
    preTuringchainBalance=$(${TuringchainCli} x2ethereum balance -s 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv -t testc | jq ".res" | jq ".[]" | jq ".balance" | sed 's/\"//g')
    ...
    nowTuringchainBalance=$(${TuringchainCli} x2ethereum balance -s 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv -t testc | jq ".res" | jq ".[]" | jq ".balance" | sed 's/\"//g')
    diff=$((nowTuringchainBalance - preTuringchainBalance))
    check_number "${diff}" 7
}

loop_send_burn_erc20() {
    echo -e "${GRE}=========== Turingchain Burn Erc20 begin ===========${NOC}"
    preTuringchainBalance=$(${TuringchainCli} x2ethereum balance -s 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv -t testc | jq ".res" | jq ".[]" | jq ".balance" | sed 's/\"//g')

    i=0
    while [[ i -lt ${#privateKeys[@]} ]]; do
        preEthBalance[i]=$(${CLIA} relayer ethereum balance -o "${ethAddress[i]}" -t "${tokenAddr}" | jq -r ".balance")
        ethTxHash=$(${TuringchainCli} send x2ethereum burn -a 1 -r ${ethAddress[i]} -t testc -q "${tokenAddr}" --node_addr "${ethUrl}" -k 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv)
        echo ${i} "burn turingchain tx hash:" "${ethTxHash}"
        # shellcheck disable=SC2219
        let i++
    done
