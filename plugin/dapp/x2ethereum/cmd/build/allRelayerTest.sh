...
function TestTuringchainToEthAssets() {
    ...
    result=$(${CLIA} relayer ethereum token4turingchain -s coins.trc)
    ...
    hash=$(${TuringchainCli} send x2ethereum lock -a 5 -t coins.trc -r ${ethReceiverAddr1} -q "${tokenAddrTrc}" --node_addr "${ethUrl}" -k 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv)
    block_wait "${TuringchainCli}" $((maturityDegree + 2))
    check_tx "${TuringchainCli}" "${hash}"
...
function TestETH2TuringchainAssets() {
    ...
    hash=$(${TuringchainCli} send x2ethereum burn -a 0.1 -t eth -r ${ethReceiverAddr2} --node_addr "${ethUrl}" -k 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv)
    block_wait "${TuringchainCli}" $((maturityDegree + 2))
    check_tx "${TuringchainCli}" "${hash}"
...
function TestETH2TuringchainErc20() {
    ...
    hash=$(${TuringchainCli} send x2ethereum burn -a 100 -t "${tokenSymbol}" -r ${ethReceiverAddr2} -q "${tokenAddr}" --node_addr "${ethUrl}" -k "${turingchainValidator1}")
    block_wait "${TuringchainCli}" $((maturityDegree + 2))
    check_tx "${TuringchainCli}" "${hash}"
...
function TestTuringchainToEthAssetsKill() {
    ...
    # turingchain lock trc
    hash=$(${TuringchainCli} send x2ethereum lock -a 5 -t coins.trc -r ${ethReceiverAddr2} -q "${tokenAddrTrc}" --node_addr "${ethUrl}" -k 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv)
    block_wait "${TuringchainCli}" $((maturityDegree + 2))
    check_tx "${TuringchainCli}" "${hash}"
    ...
    # eth burn
    result=$(${CLIA} relayer ethereum burn -m 5 -k "${ethReceiverAddrKey2}" -r "${turingchainValidator1}" -t "${tokenAddrTrc}")
    cli_ret "${result}" "burn"
...
function TestETH2TuringchainErc20Kill() {
    ...
    # lock 100
    result=$(${CLIA} relayer ethereum lock -m 100 -k "${ethReceiverAddrKey1}" -r "${turingchainValidator1}" -t "${tokenAddr2}")
    cli_ret "${result}" "lock"
    ...
    result=$(${TuringchainCli} x2ethereum balance -s "${turingchainValidator1}" -t "${tokenSymbol}" -a "${tokenAddr2}" | jq ".res" | jq ".[]")
    balance_ret "${result}" "0"
    ...
    # turingchain burn 100
    hash=$(${TuringchainCli} send x2ethereum burn -a 100 -t "${tokenSymbol}" -r ${ethReceiverAddr2} -q "${tokenAddr2}" --node_addr "${ethUrl}" -k "${turingchainValidator1}")
    block_wait "${TuringchainCli}" $((maturityDegree + 2))
    check_tx "${TuringchainCli}" "${hash}"
...
function AllRelayerMainTest() {
    set +e
    docker_turingchain_ip=$(get_docker_addr "${dockerNamePrefix}_turingchain_1")
    TuringchainCli="./turingchain-cli --rpc_laddr http://${docker_turingchain_ip}:9671"
