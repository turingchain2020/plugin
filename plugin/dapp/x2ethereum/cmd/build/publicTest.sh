...
function exit_cp_file() {
    ...
    docker cp "${dockerNamePrefix}_turingchain_1":/root/logs/turingchain.log "${dirName}/turingchain.log"
    docker logs "${dockerNamePrefix}_turingchain_1" | tail -n 1000
...
function copyErrLogs() {
    ...
    if [ -n "$CASE_ERR" ]; then
        ...
        docker cp "${dockerNamePrefix}_turingchain_1":/root/logs/turingchain.log "${dirName}/turingchain_rpc.log"
        docker logs "${dockerNamePrefix}_turingchain_1" | tail -n 1000
    fi
}
...
# turingchain 区块等待 $1:cli 路径  $2:等待高度
function block_wait() {
    set +x
    local CLI=${1}
    ...
    count=$((count + 1))
    set -x
    echo -e "${GRE}turingchain wait new block $count s, cur height=$expect,old=$cur_height${NOC}"
}
...
function check_tx() {
    ...
    while true; do
        ...
        if [[ ${count} -ge 100 ]]; then
            echo "turingchain query tx for too long"
            break
        fi
    done
...
function updata_relayer_a_toml() {
    ...
    local turingchainHost=$(get_docker_addr "${dockerNamePrefix}_turingchain_1")
    if [[ ${turingchainHost} == "" ]]; then
        echo -e "${RED}turingchainHost is empty${NOC}"
        exit_cp_file
    fi

    local line=$(delete_line_show "${file}" "turingchainHost")
    # 在第 line 行后面 新增合约地址
    sed -i ''"${line}"' a turingchainHost="http://'"${turingchainHost}"':9671"' "${file}"
...
function updata_relayer_toml_ropston() {
    ...
    local turingchainHost=127.0.0.1
