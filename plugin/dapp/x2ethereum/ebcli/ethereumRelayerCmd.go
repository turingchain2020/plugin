...
func EthereumRelayerCmd() *cobra.Command {
	...
	cmd.AddCommand(
		ImportTuringchainPrivateKeyCmd(),
		GenEthPrivateKeyCmd(),
		ShowValidatorsAddrCmd(),
		ShowTuringchainTxsHashCmd(),
...
//ImportTuringchainPrivateKeyCmd ...
func ImportTuringchainPrivateKeyCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "import_turingchainprivatekey",
		Short: "import turingchain private key to sign txs to be submitted to turingchain",
		Run:   importTuringchainPrivatekey,
	}
	addImportTuringchainPrivateKeyFlags(cmd)
	return cmd
}

func addImportTuringchainPrivateKeyFlags(cmd *cobra.Command) {
	cmd.Flags().StringP("key", "k", "", "turingchain private key")
	cmd.MarkFlagRequired("key")
}

func importTuringchainPrivatekey(cmd *cobra.Command, args []string) {
	rpcLaddr, _ := cmd.Flags().GetString("rpc_laddr")
	privateKey, _ := cmd.Flags().GetString("key")
	params := privateKey

	var res rpctypes.Reply
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Manager.ImportTuringchainPrivateKey4EthRelayer", params, &res)
	ctx.Run()
}
...
func ShowValidatorsAddrCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "show_validators",
		Short: "show me the validators including ethereum and turingchain",
...
//ShowTuringchainTxsHashCmd ...
func ShowTuringchainTxsHashCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "show_turingchain_tx",
		Short: "show me the turingchain tx hashes",
		Run:   showTuringchainTxs,
	}
	return cmd
}

func showTuringchainTxs(cmd *cobra.Command, args []string) {
	rpcLaddr, _ := cmd.Flags().GetString("rpc_laddr")

	var res ebTypes.Txhashes
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Manager.ShowEthRelayer2TuringchainTxs", nil, &res)
...
func CreateBridgeTokenCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "token4turingchain",
		Short: "create new token as turingchain asset on Ethereum",
...
func BurnCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "burn",
		Short: "burn(including approve) the asset to make it unlocked on turingchain",
		Run:   Burn,
	}
	BurnFlags(cmd)
	return cmd
...
func BurnAsyncCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "burn-async",
		Short: "async burn the asset to make it unlocked on turingchain",
		Run:   BurnAsync,
	}
	BurnFlags(cmd)
	return cmd
...
func BurnFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringP("token", "t", "", "token address")
	_ = cmd.MarkFlagRequired("token")
	cmd.Flags().StringP("receiver", "r", "", "receiver address on turingchain")
...
func LockSyncCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "lock",
		Short: "lock(including approve) eth or erc20 and cross-chain transfer to turingchain",
...
func LockAsyncCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "lock-async",
		Short: "async lock eth or erc20 and cross-chain transfer to turingchain",
...
func LockEthErc20AssetFlags(cmd *cobra.Command) {
	...
	cmd.Flags().Float64P("amount", "m", float64(0), "amount")
	_ = cmd.MarkFlagRequired("amount")
	cmd.Flags().StringP("receiver", "r", "", "turingchain receiver address")
	_ = cmd.MarkFlagRequired("receiver")
}
