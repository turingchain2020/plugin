...
// ethereum ---> turingchain
// lock
func (a *action) procEth2Turingchain_lock(ethBridgeClaim *x2eTy.Eth2Turingchain) (*types.Receipt, error) {
	...
	if status.Text == x2eTy.EthBridgeStatus_SuccessStatusText {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "relay procMsgEth2Turingchain,exec=%s,sym=%s", x2eTy.X2ethereumX, ethBridgeClaim.IssuerDotSymbol)
		}
		...
		receipt.KV = append(receipt.KV, &types.KeyValue{Key: x2eTy.CalEth2TuringchainPrefix(), Value: msgEthBridgeClaimBytes})

		execlog := &types.ReceiptLog{Ty: x2eTy.TyEth2TuringchainLog, Log: types.Encode(&x2eTy.ReceiptEth2Turingchain{
			...
			TuringchainReceiver:       ethBridgeClaim.TuringchainReceiver,
...
// turingchain -> ethereum
// 返还在turingchain上生成的erc20代币
func (a *action) procTuringchainToEth_burn(msgBurn *x2eTy.TuringchainToEth) (*types.Receipt, error) {
	receipt, err := a.checkConsensusThreshold()
	if err != nil {
		return nil, err
	}
	...
	execlog := &types.ReceiptLog{Ty: x2eTy.TyWithdrawTuringchainLog, Log: types.Encode(&x2eTy.ReceiptTuringchainToEth{
		TuringchainSender:    a.fromaddr,
	...
	receipt.KV = append(receipt.KV, &types.KeyValue{Key: x2eTy.CalWithdrawTuringchainPrefix(), Value: msgBurnBytes})

	receipt.Ty = types.ExecOk
	return receipt, nil
}

func (a *action) procTuringchainToEth_lock(msgLock *x2eTy.TuringchainToEth) (*types.Receipt, error) {
	receipt, err := a.checkConsensusThreshold()
	if err != nil {
		return nil, err
	}
	...
	execlog := &types.ReceiptLog{Ty: x2eTy.TyTuringchainToEthLog, Log: types.Encode(&x2eTy.ReceiptTuringchainToEth{
		TuringchainSender:    a.fromaddr,
	...
	receipt.KV = append(receipt.KV, &types.KeyValue{Key: x2eTy.CalTuringchainToEthPrefix(), Value: msgLockBytes})
...
// ethereum -> turingchain
// burn
func (a *action) procEth2Turingchain_burn(withdrawEth *x2eTy.Eth2Turingchain) (*types.Receipt, error) {
	...
	if status.Text == x2eTy.EthBridgeStatus_SuccessStatusText {
		...
		execlog := &types.ReceiptLog{Ty: x2eTy.TyWithdrawEthLog, Log: types.Encode(&x2eTy.ReceiptEth2Turingchain{
			...
			TuringchainReceiver:       withdrawEth.TuringchainReceiver,
