...
func NewOracleClaimContent(turingchainReceiver string, amount string, claimType, decimals int64) x2eTy.OracleClaimContent {
	return x2eTy.OracleClaimContent{
		TuringchainReceiver: turingchainReceiver,
		Amount:          amount,
		ClaimType:       claimType,
		Decimals:        decimals,
	}
...
//CreateOracleClaimFromEthClaim 通过ethturingchain结构构造一个OracleClaim结构，包括生成唯一的ID
func CreateOracleClaimFromEthClaim(ethClaim x2eTy.Eth2Turingchain) (x2eTy.OracleClaim, error) {
	...
	claimContent := NewOracleClaimContent(ethClaim.TuringchainReceiver, ethClaim.Amount, ethClaim.ClaimType, ethClaim.Decimals)
