...
import (
	...
	"github.com/turingchain2020/turingchain/common/db/mocks"
	...
	types2 "github.com/turingchain2020/plugin/plugin/dapp/x2ethereum/types"
	"github.com/stretchr/testify/mock"
	"github.com/stretchr/testify/suite"
)

var chainTestCfg = types.NewTuringchainConfig(types.GetDefaultCfgstring())
...
var (
	turingchainReceiver       = "1BqP2vHkYNjSgdnTqm7pGbnphLhtEhuJFi"
...
func (x *suiteX2Ethereum) Test_3_ModifyAndRemoveValidator() {
	...
	msg, err := x.x2eth.Query_GetValidators(&types2.QueryValidatorsParams{Validator: turingchainReceiver})
	x.NoError(err)
	...
	_, err = x.x2eth.Query_GetValidators(&types2.QueryValidatorsParams{Validator: turingchainReceiver})
	x.Equal(err, types2.ErrInvalidValidator)
}

func (x *suiteX2Ethereum) Test_4_Eth2Turingchain() {
	_, err := x.x2eth.Query_GetTotalPower(&types2.QueryTotalPowerParams{})
	if err == types.ErrNotFound {
		x.Test_2_AddValidator()
	}

	payload := &types2.Eth2Turingchain{
		EthereumChainID:       0,
		...
		TuringchainReceiver:       addValidator1,
		ValidatorAddress:      addValidator1,
		Amount:                "10",
		ClaimType:             int64(types2.LockClaimType),
	}

	receipt, err := x.action.procEth2Turingchain_lock(payload)
	x.NoError(err)
	x.setDb(receipt)
	...
	payload1 := &types2.TuringchainToEth{
		TokenContract:    tokenContractAddress,
		TuringchainSender:    addValidator1,
	...
	receipt, err = x.action.procTuringchainToEth_burn(payload1)
	x.NoError(err)
	x.setDb(receipt)
...
func (x *suiteX2Ethereum) Test_5_TuringchainToEth() {
	_, err := x.x2eth.Query_GetTotalPower(&types2.QueryTotalPowerParams{})
	if err == types.ErrNotFound {
		x.Test_2_AddValidator()
	}
	msgLock := &types2.TuringchainToEth{
		TokenContract:    tokenContractAddress,
		TuringchainSender:    addValidator1,
		EthereumReceiver: ethereumAddr,
		Amount:           "5",
		IssuerDotSymbol:  "coins.trc",
	}

	receipt, err := x.action.procTuringchainToEth_lock(msgLock)
	x.NoError(err)
	x.setDb(receipt)

	x.queryGetSymbolTotalAmount("coins.trc", tokenContractAddress, 5, false)
	x.queryGetSymbolTotalAmountByTxType(2, "coins.trc", "lock", tokenContractAddress, "5")
	...
	receipt1, err := x.action.procEth2Turingchain_burn(payload)
	x.NoError(err)
	x.setDb(receipt1)
	...
	x.queryGetSymbolTotalAmount("coins.trc", tokenContractAddress, 1, false)
	x.queryGetSymbolTotalAmountByTxType(1, "coins.trc", "withdraw", tokenContractAddress, "4")
...
func (x *suiteX2Ethereum) setDb(receipt *turingchaintypes.Receipt) {
...
func (x *suiteX2Ethereum) queryGetSymbolTotalAmount(tokenSymbol, tokenAddress string, equal int64, eth2turingchain bool) {
