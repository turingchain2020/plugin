...
import (
	...
	types2 "github.com/turingchain2020/turingchain/rpc/types"
	"github.com/turingchain2020/turingchain/system/dapp/commands"
	"github.com/turingchain2020/turingchain/types"
	"github.com/turingchain2020/plugin/plugin/dapp/x2ethereum/ebcli/buildflags"
	"github.com/turingchain2020/plugin/plugin/dapp/x2ethereum/ebrelayer/utils"
	types3 "github.com/turingchain2020/plugin/plugin/dapp/x2ethereum/types"
...
func Cmd() *cobra.Command {
	...
	cmd.AddCommand(
		CreateRawWithdrawTuringchainTxCmd(),
		CreateRawTuringchainToEthTxCmd(),
...
//CreateRawWithdrawTuringchainTxCmd Burn
func CreateRawWithdrawTuringchainTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "burn",
		Short: "Create a burn tx in turingchain,withdraw turingchainToEth",
		Run:   burn,
	}

	addTuringchainToEthFlags(cmd)

	return cmd
}

func addTuringchainToEthFlags(cmd *cobra.Command) {
	cmd.Flags().StringP("contract", "q", "", "token contract address,nil for ETH")

	cmd.Flags().StringP("symbol", "t", "", "token symbol in turingchain,coins.trc etc.")
...
func burn(cmd *cobra.Command, args []string) {
	...
	params := &types3.TuringchainToEth{
	...
	createTx(cmd, payLoad, types3.NameWithdrawTuringchainAction)
}

//CreateRawTuringchainToEthTxCmd Lock
func CreateRawTuringchainToEthTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "lock",
		Short: "Create a lock tx in turingchain,create a turingchainToEth tx",
		Run:   lock,
	}
...
func lock(cmd *cobra.Command, args []string) {
	...
	createTx(cmd, payLoad, types3.NameTuringchainToEthAction)
}
...
func CreateTransferCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "transfer",
		Short: "Create a transfer tx in turingchain",
		Run:   transfer,
	}
...
func CreateRawAddValidatorTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "add",
		Short: "Create a add validator tx in turingchain",
		Run:   addValidator,
	}
...
func CreateRawRemoveValidatorTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "remove",
		Short: "Create a remove validator tx in turingchain",
		Run:   removeValidator,
	}
...
func CreateRawModifyValidatorTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "modify",
		Short: "Create a modify validator tx in turingchain",
		Run:   modify,
	}
...
func CreateRawSetConsensusTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "setconsensus",
		Short: "Create a set consensus threshold tx in turingchain",
		Run:   setConsensus,
	}
...
func queryRelayerBalance(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Turingchain.Query", query, channel)
	ctx.Run()
}
...
func createTx(cmd *cobra.Command, payLoad []byte, action string) {
	...
	var res string
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Turingchain.CreateTransaction", pm, &res)
