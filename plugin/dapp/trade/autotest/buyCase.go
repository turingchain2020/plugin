...
type BuyCase struct {
	...
	TrcAmount   string `toml:"trcAmount"`
}
...
func (pack *BuyPack) checkBalance(txInfo map[string]interface{}) bool {
	...
	logBuyTrc := logArr[1].(map[string]interface{})["log"].(map[string]interface{})
	logSellTrc := logArr[2].(map[string]interface{})["log"].(map[string]interface{})
	...
	trcAmount, _ := strconv.ParseFloat(interCase.TrcAmount, 64)

	pack.FLog.Info("BuyBalanceDetails", "ID", pack.PackID,
		"Fee", feeStr, "TokenAmount", interCase.TokenAmount, "TrcAmount", interCase.TrcAmount,
		"SellerTrcPrev", logSellTrc["prev"].(map[string]interface{})["balance"].(string),
		"SellerTrcCurr", logSellTrc["current"].(map[string]interface{})["balance"].(string),
		"BuyerTrcPrev", logBuyTrc["prev"].(map[string]interface{})["balance"].(string),
		"BuyerTrcCurr", logBuyTrc["current"].(map[string]interface{})["balance"].(string),
	...
	return types.CheckBalanceDeltaWithAddr(logFee, interCase.From, -fee) &&
		types.CheckBalanceDeltaWithAddr(logBuyTrc, interCase.From, -trcAmount) &&
		types.CheckBalanceDeltaWithAddr(logSellTrc, interCase.To, trcAmount) &&
