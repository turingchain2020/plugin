...
import (
	...
	pty "github.com/turingchain2020/plugin/plugin/dapp/trade/types"
...
var (
	...
	turingchainTestCfg = types.NewTuringchainConfig(strings.Replace(types.GetDefaultCfgstring(), "Title=\"local\"", "Title=\"turingchain\"", 1))
)

func init() {
	Init(pty.TradeX, turingchainTestCfg, nil)
}
...
func TestTrade_Exec_SellLimit(t *testing.T) {
	...
	env := execEnv{
		1539918074,
		turingchainTestCfg.GetDappFork("trade", pty.ForkTradePriceX),
		2,
		1539918074,
		"hash",
	}
	...
	accB := account.NewCoinsAccount(turingchainTestCfg)
	accB.SetDB(kvdb)
	accB.SaveExecAccount(address.ExecAddress("trade"), &accountB)

	accA, _ := account.NewAccountDB(turingchainTestCfg, AssetExecToken, Symbol, kvdb)
	accA.SaveExecAccount(address.ExecAddress("trade"), &accountA)

	api := new(apimock.QueueProtocolAPI)
	api.On("GetConfig", mock.Anything).Return(turingchainTestCfg, nil)
	driver := newTrade()
	driver.SetAPI(api)
	...
	tx, _ := pty.CreateRawTradeSellTx(turingchainTestCfg, sell)
	...
	tx, _ = pty.CreateRawTradeBuyTx(turingchainTestCfg, buy)
...
func TestTrade_Exec_BuyLimit(t *testing.T) {
	...
	accB, _ := account.NewAccountDB(turingchainTestCfg, AssetExecToken, SymbolA, stateDB)
	accB.SetDB(stateDB)
	accB.SaveExecAccount(address.ExecAddress("trade"), &accountB)

	accA, _ := account.NewAccountDB(turingchainTestCfg, AssetExecPara, Symbol, stateDB)
	accA.SaveExecAccount(address.ExecAddress("trade"), &accountA)
	...
	tx, _ := pty.CreateRawTradeBuyLimitTx(turingchainTestCfg, buy)
	tx, _ = signTx(tx, PrivKeyB)
	...
	tx, _ = pty.CreateRawTradeSellMarketTx(turingchainTestCfg, sell)
	tx, _ = signTx(tx, PrivKeyA)
	receipt, err = driver.Exec(tx, env.index)
	if err != nil {
		assert.Nil(t, err, "exec failed")
...
func TestTradeSellFixAssetDB(t *testing.T) {
	turingchainTestCfg.SetDappFork(pty.TradeX, pty.ForkTradeAssetX, int64(10))
	turingchainTestCfg.SetDappFork(pty.TradeX, pty.ForkTradeIDX, int64(10))
	turingchainTestCfg.SetDappFork(pty.TradeX, pty.ForkTradeFixAssetDBX, int64(20))
	turingchainTestCfg.SetDappFork(pty.TradeX, pty.ForkTradePriceX, int64(30))
	...
	envA := execEnv{
		1539918074,
		turingchainTestCfg.GetDappFork("trade", pty.ForkTradeAssetX) - 1,
		2,
		1539918074,
		"hash",
	}
	...
	envB := execEnv{
		1539918074,
		turingchainTestCfg.GetDappFork("trade", pty.ForkTradeFixAssetDBX) - 1,
		2,
		1539918074,
		"hash",
	}
	...
	envC := execEnv{
		1539918074,
		turingchainTestCfg.GetDappFork("trade", pty.ForkTradeFixAssetDBX),
		2,
		1539918074,
		"hash",
	}
	...
	tx, _ = pty.CreateRawTradeBuyTx(turingchainTestCfg, buyB)
	tx, _ = signTx(tx, PrivKeyB)
