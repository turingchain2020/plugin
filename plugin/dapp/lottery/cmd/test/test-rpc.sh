...
init() {
    ...
    if [[ $ispara == true ]]; then
        lottExecAddr=$(curl -ksd '{"method":"Turingchain.ConvertExectoAddr","params":[{"execname":"user.p.para.lottery"}]}' ${MAIN_HTTP} | jq -r ".result")
        turingchain_SignAndSendTx "${lottery_addCreator_unsignedTx_para}" "${lottery_creator_priv}" ${MAIN_HTTP}
    else
        lottExecAddr=$(curl -ksd '{"method":"Turingchain.ConvertExectoAddr","params":[{"execname":"lottery"}]}' ${MAIN_HTTP} | jq -r ".result")
        turingchain_SignAndSendTx "${lottery_addCreator_unsignedTx}" "${lottery_creator_priv}" ${MAIN_HTTP}
    fi
    echo "lottExecAddr=$lottExecAddr"

    local main_ip=${MAIN_HTTP//8901/9671}
    turingchain_ImportPrivkey "0x8223b757a5d0f91b12e7af3b9666ca33be47fe63e1502987b0537089aaf90bc1" "1FLh9wBS2rat1mUS4G95hRpJt6yHYy5nHF" "lottery1" "${main_ip}"
    turingchain_ImportPrivkey "0xbfccb96690e0a1f89748b321f85b03e14bda0cb3d5d19f255ff0b9b0ffb624b3" "1UWE6NfXPR7eNAjYgT4HMERp7cMMi486E" "lottery2" "$main_ip"
    ...
    if [ "$ispara" == false ]; then
        ...
        turingchain_ImportPrivkey "0x8223b757a5d0f91b12e7af3b9666ca33be47fe63e1502987b0537089aaf90bc1" "1FLh9wBS2rat1mUS4G95hRpJt6yHYy5nHF" "lottery1" "$para_ip"
        turingchain_ImportPrivkey "0xbfccb96690e0a1f89748b321f85b03e14bda0cb3d5d19f255ff0b9b0ffb624b3" "1UWE6NfXPR7eNAjYgT4HMERp7cMMi486E" "lottery2" "$para_ip"
...
lottery_LotteryCreate() {
    #创建交易
    priv=$1
    req='{"method":"Turingchain.CreateTransaction","params":[{"execer":"lottery","actionName":"LotteryCreate","payload":{"purBlockNum":'"$purNum"',"drawBlockNum":'"$drawNum"', "opRewardRatio":'"$opRatio"',"devRewardRatio":'"$devRatio"',"fee":1000000}}]}'
    turingchain_Http "$req" ${MAIN_HTTP} '(.error|not)' "$FUNCNAME" ".result"

    #发送交易
    turingchain_SignAndSendTx "${RETURN_RESP}" "${priv}" ${MAIN_HTTP}
...
lottery_LotteryBuy() {
    ...
    number=$3
    way=$4
    req='{"method":"Turingchain.CreateTransaction","params":[{"execer":"lottery","actionName":"LotteryBuy","payload":{"lotteryId":"'"$gID"'","amount":'"$amount"',"number":'"$number"',"way":'"$way"',"fee":1000000}}]}'
    turingchain_Http "$req" ${MAIN_HTTP} '(.error|not)' "$FUNCNAME" ".result"
...
lottery_LotteryDraw() {
    #创建交易
    priv=$1
    req='{"method":"Turingchain.CreateTransaction","params":[{"execer":"lottery","actionName":"LotteryDraw","payload":{"lotteryId":"'"$gID"'","fee":1000000}}]}'
    turingchain_Http "$req" ${MAIN_HTTP} '(.error|not)' "$FUNCNAME" ".result"
    #发送交易
    turingchain_SignAndSendTx "${RETURN_RESP}" "${priv}" ${MAIN_HTTP}
}
...
lottery_LotteryClose() {
    #创建交易
    priv=$1
    req='{"method":"Turingchain.CreateTransaction","params":[{"execer":"lottery","actionName":"LotteryClose","payload":{"lotteryId":"'"$gID"'","fee":1000000}}]}'
    turingchain_Http "$req" ${MAIN_HTTP} '(.error|not)' "$FUNCNAME" ".result"
    #发送交易
    turingchain_SignAndSendTx "${RETURN_RESP}" "${priv}" ${MAIN_HTTP}
}
...
lottery_GetLotteryNormalInfo() {
    gameID=$1
    addr=$2
    req='{"method":"Turingchain.Query","params":[{"execer":"lottery","funcName":"GetLotteryNormalInfo","payload":{"lotteryId":"'"$gameID"'"}}]}'
...
lottery_GetLotteryCurrentInfo() {
    ...
    status=$2
    amount=$3
    req='{"method":"Turingchain.Query","params":[{"execer":"lottery","funcName":"GetLotteryCurrentInfo","payload":{"lotteryId":"'"$gameID"'"}}]}'
    ...
    turingchain_Http "$req" ${MAIN_HTTP} "$resok" "$FUNCNAME" ".result.luckyNumber"
...
lottery_GetLotteryPurchaseAddr() {
    gameID=$1
    count=$2
    req='{"method":"Turingchain.Query","params":[{"execer":"lottery","funcName":"GetLotteryPurchaseAddr","payload":{"lotteryId":"'"$gameID"'"}}]}'
...
lottery_GetLotteryHistoryLuckyNumber() {
    ...
    count=$2
    lucky=$3
    req='{"method":"Turingchain.Query","params":[{"execer":"lottery","funcName":"GetLotteryHistoryLuckyNumber","payload":{"lotteryId":"'"$gameID"'"}}]}'
...
lottery_GetLotteryRoundLuckyNumber() {
    ...
    round=$2
    lucky=$3
    req='{"method":"Turingchain.Query","params":[{"execer":"lottery","funcName":"GetLotteryRoundLuckyNumber","payload":{"lotteryId":"'"$gameID"'", "round":['"$round"']}}]}'
...
lottery_GetLotteryHistoryBuyInfo() {
    ...
    count=$3
    number=$4
    req='{"method":"Turingchain.Query","params":[{"execer":"lottery","funcName":"GetLotteryHistoryBuyInfo","payload":{"lotteryId":"'"$gameID"'", "addr":"'"$addr"'"}}]}'
...
lottery_GetLotteryBuyRoundInfo() {
    ...
    count=$4
    number=$5
    req='{"method":"Turingchain.Query","params":[{"execer":"lottery","funcName":"GetLotteryBuyRoundInfo","payload":{"lotteryId":"'"$gameID"'", "addr":"'"$addr"'", "round":'"$round"'}}]}'
    resok='(.error|not) and (.result.records | length == '"$count"') and (.result.records[0].number == "'"$number"'")'
    turingchain_Http "$req" ${MAIN_HTTP} "$resok" "$FUNCNAME"
}
...
lottery_GetLotteryHistoryGainInfo() {
    ...
    count=$3
    amount=$4
    req='{"method":"Turingchain.Query","params":[{"execer":"lottery","funcName":"GetLotteryHistoryGainInfo","payload":{"lotteryId":"'"$gameID"'", "addr":"'"$addr"'"}}]}'
...
lottery_GetLotteryRoundGainInfo() {
    ...
    round=$3
    amount=$4
    req='{"method":"Turingchain.Query","params":[{"execer":"lottery","funcName":"GetLotteryRoundGainInfo","payload":{"lotteryId":"'"$gameID"'", "addr":"'"$addr"'", "round":'"$round"'}}]}'
...
function run_testcases() {
    ...
    #给游戏合约中转帐
    turingchain_SendToAddress "${gameAddr1}" "${lottExecAddr}" 500000000 "${MAIN_HTTP}"
    turingchain_SendToAddress "${gameAddr2}" "${lottExecAddr}" 500000000 "${MAIN_HTTP}"
    ...
    #游戏开奖
    M_HTTP=${MAIN_HTTP//8901/9671}
    turingchain_BlockWait ${drawNum} "${M_HTTP}"
...
function main() {
    turingchain_RpcTestBegin lottery
    MAIN_HTTP="$1"
    echo "main_ip=$MAIN_HTTP"

    init
    run_testcases
    turingchain_RpcTestRst lottery "$CASE_ERR"
}
