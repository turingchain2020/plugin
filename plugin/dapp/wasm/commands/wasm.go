...
import (
	...
	wasmtypes "github.com/turingchain2020/plugin/plugin/dapp/wasm/types"
...
func cmdCreateContract() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "create",
		Short: "publish a new contract on turingchain",
		Run:   createContract,
	}
	cmd.Flags().StringP("name", "n", "", "contract name")
...
func cmdCallContract() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "call",
		Short: "call contract on turingchain",
		Run:   callContract,
	}
	cmd.Flags().StringP("name", "n", "", "contract name")
...
func checkContract(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Turingchain.Query", params, &resp)
	ctx.Run()
}
