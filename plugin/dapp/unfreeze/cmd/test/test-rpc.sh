...
function query_unfreezeID() {
    ...
    local times=10
    while true; do
        req='{"method":"Turingchain.QueryTransaction","params":[{"hash":"'"$txhash"'"}]}'
...
function init() {
    ...
    turingchain_ImportPrivkey "${beneficiary_key}" "${beneficiary}" "unfreeze_beneficiary" "${MAIN_HTTP}"
    turingchain_ImportPrivkey "${owner_key}" "${owner}" "unfreeze_owner" "${MAIN_HTTP}"

    turingchain_applyCoins "${beneficiary}" 10000000000 "${MAIN_HTTP}"
    turingchain_applyCoins "${owner}" 10000000000 "${MAIN_HTTP}"
    ...
    if [ "$ispara" == true ]; then
        ...
        turingchain_applyCoins "${beneficiary}" 10000000000 "${main_ip}"
        turingchain_applyCoins "${owner}" 10000000000 "${main_ip}"
    fi

    exec_addr=$(curl -ksd '{"method":"Turingchain.ConvertExectoAddr","params":[{"execname":"'${exec_name}'"}]}' ${MAIN_HTTP} | jq -r ".result")
    echo "exec_addr=${exec_addr}"
    turingchain_SendToAddress "$owner" "$exec_addr" 5000000000 "${MAIN_HTTP}"
    turingchain_SendToAddress "$beneficiary" "$exec_addr" 5000000000 "${MAIN_HTTP}"
    turingchain_BlockWait 1 "${MAIN_HTTP}"
}
...
function CreateRawUnfreezeCreate() {
    ...
    turingchain_SignAndSendTx "$RETURN_RESP" "$owner_key" "${MAIN_HTTP}"
    query_unfreezeID
}
...
function CreateRawUnfreezeWithdraw() {
    ...
    turingchain_Http "$req" ${MAIN_HTTP} '(.error|not) and (.result != null)' "$FUNCNAME" ".result"
    turingchain_SignAndSendTx "$RETURN_RESP" "${beneficiary_key}" "${MAIN_HTTP}"
}
...
function rpc_test() {
    turingchain_RpcTestBegin unfreeze
    MAIN_HTTP="$1"
    echo "main_ip=$MAIN_HTTP"

    init
    run_testcases
    turingchain_RpcTestRst unfreeze "$CASE_ERR"
}
