...
import (
	"github.com/turingchain2020/turingchain/common/db/table"
	...
	pty "github.com/turingchain2020/plugin/plugin/dapp/collateralize/types"
	issuanceE "github.com/turingchain2020/plugin/plugin/dapp/issuance/types"
	tokenE "github.com/turingchain2020/plugin/plugin/dapp/token/executor"
)
...
// 根据最近抵押物价格计算需要冻结的TRC数量
func getTrcNumToFrozen(value int64, price int64, ratio int64) (int64, error) {
	if price == 0 {
		clog.Error("Trc price should greate to 0")
		return 0, pty.ErrPriceInvalid
	}

	trcValue := (value * 1e4) / (price * ratio)
	return trcValue * 1e4, nil
}
...
// CollateralizeBorrow 用户质押trc借出ccny
func (action *Action) CollateralizeBorrow(borrow *pty.CollateralizeBorrow) (*types.Receipt, error) {
	...
	trcFrozen, err := getTrcNumToFrozen(borrow.GetValue(), lastPrice, coll.LiquidationRatio)
	if err != nil {
		clog.Error("CollateralizeBorrow.getTrcNumToFrozen", "CollID", coll.CollateralizeId, "addr", action.fromaddr, "execaddr", action.execaddr, "error", err)
		return nil, err
	}

	// 检查抵押物账户余额
	if !action.CheckExecAccountBalance(action.fromaddr, trcFrozen, 0) {
		clog.Error("CollateralizeBorrow.CheckExecAccountBalance", "CollID", coll.CollateralizeId, "addr", action.fromaddr, "execaddr", action.execaddr, "balance", trcFrozen, "error", types.ErrNoBalance)
		return nil, types.ErrNoBalance
	}

	// 抵押物转账
	receipt, err := action.coinsAccount.ExecTransfer(action.fromaddr, coll.CreateAddr, action.execaddr, trcFrozen)
	if err != nil {
		clog.Error("CollateralizeBorrow.ExecTransfer", "addr", action.fromaddr, "execaddr", action.execaddr, "amount", trcFrozen)
		return nil, err
	}
	logs = append(logs, receipt.Logs...)
	kv = append(kv, receipt.KV...)
	...
	// 抵押物冻结
	receipt, err = action.coinsAccount.ExecFrozen(coll.CreateAddr, action.execaddr, trcFrozen)
	if err != nil {
		clog.Error("CollateralizeBorrow.Frozen", "addr", coll.CreateAddr, "execaddr", action.execaddr, "amount", trcFrozen)
		return nil, err
	}
	logs = append(logs, receipt.Logs...)
	kv = append(kv, receipt.KV...)
	...
	borrowRecord.CollateralValue = trcFrozen
