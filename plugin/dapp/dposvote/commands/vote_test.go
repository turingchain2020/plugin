...
import (
	...
	"github.com/turingchain2020/plugin/plugin/consensus/dpos"
	ttypes "github.com/turingchain2020/plugin/plugin/consensus/dpos/types"
...
[rpc]
jrpcBindAddr="localhost:9671"
grpcBindAddr="localhost:9672"
whitelist=["127.0.0.1"]
jrpcFuncWhitelist=["*"]
grpcFuncWhitelist=["*"]
...
func testCmd(cmd *cobra.Command) {
	var rootCmd = &cobra.Command{
		Use:   "turingchain-cli",
		Short: "turingchain client tools",
	}
	...
	rootCmd.PersistentFlags().String("rpc_laddr", "http://127.0.0.1:9672", "http url")
	rootCmd.AddCommand(cmd)

	rootCmd.SetArgs([]string{"dpos", "regist", "--address", validatorAddr, "--pubkey", strPubkey, "--ip", "127.0.0.1", "--rpc_laddr", "http://127.0.0.1:9671"})
	rootCmd.Execute()

	rootCmd.SetArgs([]string{"dpos", "cancelRegist", "--address", validatorAddr, "--pubkey", strPubkey, "--rpc_laddr", "http://127.0.0.1:9671"})
	rootCmd.Execute()

	rootCmd.SetArgs([]string{"dpos", "reRegist", "--address", validatorAddr, "--pubkey", strPubkey, "--ip", "127.0.0.1", "--rpc_laddr", "http://127.0.0.1:9671"})
	rootCmd.Execute()
