...
func regist(cmd *cobra.Command, args []string) {
	...
	var res string
	ctx := jsonrpc.NewRPCCtx(rpcLaddr, "Turingchain.CreateTransaction", params, &res)
...
func candidatorQuery(cmd *cobra.Command, args []string) {
	...
	case "topN":
		...
		ctx := jsonrpc.NewRPCCtx(rpcLaddr, "Turingchain.Query", params, &res)
		ctx.Run()
...
func createFiles(cmd *cobra.Command, args []string) {
	...
	genDoc := ttypes.GenesisDoc{
		ChainID:     fmt.Sprintf("turingchain-%v", RandStr(6)),
