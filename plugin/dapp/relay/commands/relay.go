...
func showBtcHeadHeightList(cmd *cobra.Command, args []string) {
	...
	err = rpc.Call("Turingchain.Query", params, &res)
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		return
	}
...
// ShowBTCHeadCurHeightCmd show BTC head current height in turingchain
...
func addExchangeFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringP("coin", "c", "", "coin to exchange by TRC, like BTC,ETH, default BTC")
	...
	cmd.Flags().Float64P("trc_amount", "b", 0, "exchange amount of TRC")
	cmd.MarkFlagRequired("trc_amount")
...
func relayOrder(cmd *cobra.Command, args []string) {
	...
	trcamount, _ := cmd.Flags().GetFloat64("trc_amount")

	if coinwait == 0 {
		coinwait = 1
	}

	trcUInt64 := uint64(trcamount * 1e4)
