...
func NewRelayd(config *Config) *Relayd {
	...
	client33 := NewClient33(&config.Turingchain)
	var btc BtcClient
...
func (r *Relayd) queryTuringchainWithBtcHeight() (*ty.ReplayRelayQryBTCHeadHeight, error) {
...
func (r *Relayd) syncBlockHeaders() {
	...
	if knownBtcHeight > r.firstHeaderHeight {
		ret, err := r.queryTuringchainWithBtcHeight()
		if err != nil {
			log.Error("syncBlockHeaders", "queryTuringchainWithBtcHeight error: ", err)
			return
		}

		log.Info("syncBlockHeaders", "queryTuringchainWithBtcHeight result: ", ret)
...
func (r *Relayd) transaction(payload []byte) *types.Transaction {
	...
	//turingchain的配置中获取chainID和minFee
	if r.config.TuringchainCfg != nil {
		chainID = r.config.TuringchainCfg.GetChainID()
		minFee = r.config.TuringchainCfg.GetMinTxFeeRate()
	}
