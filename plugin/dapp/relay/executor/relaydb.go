...
import (
	...
	ty "github.com/turingchain2020/plugin/plugin/dapp/relay/types"
	token "github.com/turingchain2020/plugin/plugin/dapp/token/types"
...
func (action *relayDB) create(order *ty.RelayCreate) (*types.Receipt, error) {
	...
	if order.Operation == ty.RelayOrderBuy {
		...
		receipt, err = accDb.ExecFrozen(action.fromAddr, action.execAddr, int64(lockTrcAmount))
		if err != nil {
			relaylog.Error("account.ExecFrozen relay ", "addrFrom", action.fromAddr, "execAddr", action.execAddr, "amount", lockTrcAmount)
			return nil, err
		}
	}
...
func (action *relayDB) revokeCreate(revoke *ty.RelayRevoke) (*types.Receipt, error) {
	...
	if order.Operation == ty.RelayOrderBuy {
		...
		receiptTransfer, err = accDb.ExecTransferFrozen(order.CreaterAddr, order.AcceptAddr, action.execAddr, int64(lockTrcAmount))
		if err != nil {
			relaylog.Error("revokeAccept", "from", order.AcceptAddr, "to", order.CreaterAddr, "execAddr", action.execAddr, "amount", lockTrcAmount)
			return nil, err
		}
	}
...
func (action *relayDB) accept(accept *ty.RelayAccept) (*types.Receipt, error) {
	...
	if order.Operation == ty.RelayOrderBuy {
		receipt, err = accDb.ExecFrozen(action.fromAddr, action.execAddr, int64(lockTrcAmount))
		if err != nil {
			relaylog.Error("relay accept frozen fail ", "addrFrom", action.fromAddr, "execAddr", action.execAddr, "amount", lockTrcAmount)
			return nil, err
		}
...
func (action *relayDB) revokeAccept(revoke *ty.RelayRevoke) (*types.Receipt, error) {
	...
	var receiptTransfer *types.Receipt
	if order.Operation == ty.RelayOrderBuy {
		receiptTransfer, err = accDb.ExecTransferFrozen(order.AcceptAddr, order.CreaterAddr, action.execAddr, int64(lockTrcAmount))
		if err != nil {
			relaylog.Error("revokeAccept", "from", order.AcceptAddr, "to", order.CreaterAddr, "execAddr", action.execAddr, "amount", lockTrcAmount)
			return nil, err
		}
...
func (action *relayDB) verifyTx(verify *ty.RelayVerify) (*types.Receipt, error) {
	...
	var receiptTransfer *types.Receipt
	if order.Operation == ty.RelayOrderBuy {
		receiptTransfer, err = accDb.ExecActive(order.AcceptAddr, action.execAddr, int64(lockTrcAmount))
		if err != nil {
			relaylog.Error("verify exec active", "from", order.AcceptAddr, "amount", lockTrcAmount)
			return nil, err
		}

	} else {
		receiptTransfer, err = accDb.ExecActive(order.CreaterAddr, action.execAddr, int64(lockTrcAmount))
		if err != nil {
			relaylog.Error("verify exec active", "from", order.CreaterAddr, "amount", lockTrcAmount)
			return nil, err
		}
	}
