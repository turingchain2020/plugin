...
import (
	...
	privacy "github.com/turingchain2020/plugin/plugin/dapp/privacy/crypto"
	privacytypes "github.com/turingchain2020/plugin/plugin/dapp/privacy/types"
)
...
func generateOuts(viewpubTo, spendpubto, viewpubChangeto, spendpubChangeto *[32]byte, transAmount, selectedAmount, fee int64) (*privacytypes.PrivacyOutput, error) {
	decomDigit := decomposeAmount2digits(transAmount, privacytypes.TRCDustThreshold)
	//计算找零
	changeAmount := selectedAmount - transAmount - fee
	var decomChange []int64
	if 0 < changeAmount {
		decomChange = decomposeAmount2digits(changeAmount, privacytypes.TRCDustThreshold)
	}
