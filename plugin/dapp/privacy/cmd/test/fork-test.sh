    ...
    while true; do
        ...
        if [ "${new_height}" -lt "${expect}" ]; then
            ...
            for ((k = 0; k < ${#names[@]}; k++)); do
                sync_status=$(docker exec "${names[$k]}" /root/turingchain-cli net is_sync)
...
function SendToPrivacyExec() {
    ...
    note=$4
    amount=$5
    #sudo docker exec -it $name ./turingchain-cli send coins transfer -k $fromAdd -t $execAdd -n $note -a $amount
...
function pub2priv() {
    ...
    amount=$5
    expire=$6
    #sudo docker exec -it $name ./turingchain-cli privacy pub2priv -f $fromAdd -p $priAdd -a $amount -n $note --expire $expire
...
function priv2priv() {
    ...
    mixcount=$6
    expire=$7
    #sudo docker exec -it $name ./turingchain-cli privacy priv2priv -f $fromAdd -p $priAdd -a $amount -n $note --expire $expire
...
function priv2pub() {
    ...
    mixcount=$6
    expire=$7
    #sudo docker exec -it $name ./turingchain-cli privacy priv2pub -f $fromAdd -t $toAdd -a $amount -n $note -m $mixcount --expire $expire
...
function createPrivacyPub2PrivTx() {
    ...
    echo "$name" trc pub2priv -p "${keypair}" -a "${amount}" -n "${note}" --expire "${expire}"
    result=$($name trc pub2priv -p "${keypair}" -a "${amount}" -n "${note}" --expire "${expire}")
    returnStr1=$result
}
...
function createPrivacyPriv2PrivTx() {
    ...
    echo "$name" trc priv2priv -p "${keypair}" -a "${amount}" -s "${sender}" -n "${note}" --expire "${expire}"
    result=$($name trc priv2priv -p "${keypair}" -a "${amount}" -s "${sender}" -n "${note}" --expire "${expire}")
    returnStr1=$result
}
...
function createPrivacyPriv2PubTx() {
    ...
    echo "$name" trc priv2pub -f "${from}" -o "${to}" -a "${amount}" -n "${note}" --expire "${expire}"
    result=$($name trc priv2pub -f "${from}" -o "${to}" -a "${amount}" -n "${note}" --expire "${expire}")
    returnStr1=$result
}
