...
import (
	...
	oraclety "github.com/turingchain2020/plugin/plugin/dapp/oracle/types"
...
func addPublishEventFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringP("sutrcpe", "s", "", "event sutrcpe, such as \"Premier League\"")
	err = cmd.MarkFlagRequired("sutrcpe")
	if err != nil {
		fmt.Printf("MarkFlagRequired sutrcpe Error: %v", err)
		return
	}
...
func publishEvent(cmd *cobra.Command, args []string) {
	...
	subType, err := cmd.Flags().GetString("sutrcpe")
	if err != nil {
		fmt.Printf("publishEvent get sutrcpe Error: %v", err)
		return
	}
	...
	var res string
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Turingchain.CreateTransaction", params, &res)
