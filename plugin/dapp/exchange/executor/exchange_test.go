...
import (
	...
	et "github.com/turingchain2020/plugin/plugin/dapp/exchange/types"
...
func TestExchange(t *testing.T) {
	...
	cfg.SetTitleOnlyForTest("turingchain")
	...
	accA, _ := account.NewAccountDB(cfg, "coins", "trc", stateDB)
	accA.SaveExecAccount(execAddr, &accountA)

	accB, _ := account.NewAccountDB(cfg, "coins", "trc", stateDB)
	accB.SaveExecAccount(execAddr, &accountB)

	accC, _ := account.NewAccountDB(cfg, "coins", "trc", stateDB)
	accC.SaveExecAccount(execAddr, &accountC)

	accD, _ := account.NewAccountDB(cfg, "coins", "trc", stateDB)
	...
	accB1, _ := account.NewAccountDB(cfg, "paracross", "coins.trc", stateDB)
	...
	*/
	...
	Exec_LimitOrder(t, &et.LimitOrder{LeftAsset: &et.Asset{Symbol: "trc", Execer: "coins"},
	...
	//根据op查询市场深度
	marketDepthList, err := Exec_QueryMarketDepth(&et.QueryMarketDepth{LeftAsset: &et.Asset{Symbol: "trc", Execer: "coins"},
	...
	orderList, err = Exec_QueryHistoryOrder(&et.QueryHistoryOrderList{LeftAsset: &et.Asset{Symbol: "trc", Execer: "coins"},
	...
	/*
			...
			反向测试
			 用例说明：
		       ...
		       4.再挂数量是15的买单
	*/
	Exec_LimitOrder(t, &et.LimitOrder{LeftAsset: &et.Asset{Symbol: "trc", Execer: "coins"}, RightAsset: &et.Asset{Execer: "paracross", Symbol: "coins.trc"}, Price: 50000000, Amount: 10 * types.Coin, Op: et.OpSell}, PrivKeyA, stateDB, kvdb, env)
	//根据地址状态查看订单
	orderList, err = Exec_QueryOrderList(et.Ordered, Nodes[0], "", stateDB, kvdb)
	assert.Nil(t, err)
	...
	Exec_LimitOrder(t, &et.LimitOrder{LeftAsset: &et.Asset{Symbol: "trc", Execer: "coins"},
		RightAsset: &et.Asset{Execer: "paracross", Symbol: "coins.trc"}, Price: 50000000, Amount: 10 * types.Coin, Op: et.OpSell}, PrivKeyA, stateDB, kvdb, env)
	//根据地址状态查看订单
	orderList, err = Exec_QueryOrderList(et.Ordered, Nodes[0], "", stateDB, kvdb)
	assert.Nil(t, err)
	...
	//根据op查询市场深度
	marketDepthList, err = Exec_QueryMarketDepth(&et.QueryMarketDepth{LeftAsset: &et.Asset{Symbol: "trc", Execer: "coins"},
		RightAsset: &et.Asset{Execer: "paracross", Symbol: "coins.trc"}, Op: et.OpSell}, stateDB, kvdb)
	assert.Equal(t, nil, err)
	//市场深度应该改变
	assert.Equal(t, 20*types.Coin, marketDepthList.List[0].GetAmount())

	Exec_LimitOrder(t, &et.LimitOrder{LeftAsset: &et.Asset{Symbol: "trc", Execer: "coins"},
		RightAsset: &et.Asset{Execer: "paracross", Symbol: "coins.trc"}, Price: 50000000, Amount: 5 * types.Coin, Op: et.OpBuy}, PrivKeyB, stateDB, kvdb, env)
	//根据地址状态查看订单
	orderList, err = Exec_QueryOrderList(et.Completed, Nodes[1], "", stateDB, kvdb)
	assert.Equal(t, nil, err)
	...
	assert.Equal(t, 5*types.Coin, order.Balance)
	Exec_LimitOrder(t, &et.LimitOrder{LeftAsset: &et.Asset{Symbol: "trc", Execer: "coins"},
		RightAsset: &et.Asset{Execer: "paracross", Symbol: "coins.trc"}, Price: 50000000, Amount: 15 * types.Coin, Op: et.OpBuy}, PrivKeyB, stateDB, kvdb, env)
	...
	*/
	...
	primaryKey = ""
	for {
		query := &et.QueryHistoryOrderList{LeftAsset: &et.Asset{Symbol: "trc", Execer: "coins"},
	...
	assert.Equal(t, et.MaxMatchCount, count)
	//相同得地址不能交易,不会撮合
	err = Exec_LimitOrder(t, &et.LimitOrder{LeftAsset: &et.Asset{Symbol: "trc", Execer: "coins"},
	...
	*/
	...
	for i := 0; i < 10; i++ {
		tx, err := CreateLimitOrder(&et.LimitOrder{LeftAsset: &et.Asset{Symbol: "trc", Execer: "coins"},
