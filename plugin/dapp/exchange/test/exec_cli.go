...
func NewExecCli() *ExecCli {
	...
	accA, _ := account.NewAccountDB(cfg, "coins", "trc", sdb)
	accA.SaveExecAccount(execAddr, accountA)

	accB, _ := account.NewAccountDB(cfg, "coins", "trc", sdb)
	accB.SaveExecAccount(execAddr, accountB)

	accC, _ := account.NewAccountDB(cfg, "coins", "trc", sdb)
	accC.SaveExecAccount(execAddr, accountC)

	accD, _ := account.NewAccountDB(cfg, "coins", "trc", sdb)
...
func (c *ExecCli) GetExecAccount(addr string, exec string, symbol string) (*types.Account, error) {
	//mavl-{coins}-{trc}-exec-{26htvcBNSEA7fZhAdLJphDwQRQJaHpyHTp}:{1JmFaA6unrCFYEWPGRi7uuXY1KthTJxJEP}
