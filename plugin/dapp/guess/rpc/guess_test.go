...
import (
	...
	"github.com/turingchain2020/turingchain/system/consensus/solo"
	...
	gty "github.com/turingchain2020/plugin/plugin/dapp/guess/types"
...
pruneHeight=10000
[wallet]
# 交易发送最低手续费，单位0.00000001TRC(1e-8),默认100000，即0.001TRC
minFee=100000
...
func Init() {
	...
	os.RemoveAll("wallet")
	os.Remove("turingchain.test.toml")

	ioutil.WriteFile("turingchain.test.toml", []byte(config), 0664)
}
...
func createConn() error {
	...
	c = types.NewTuringchainClient(conn)
	return nil
}
...
func prepareTxList(cfg *types.TuringchainConfig) *types.Transaction {
	var key string
	var value string
	var i int
...
func NormPut(cfg *types.TuringchainConfig) {
...
func sendTransferTx(cfg *types.TuringchainConfig, fromKey, to string, amount int64) bool {
...
func sendTransferToExecTx(cfg *types.TuringchainConfig, fromKey, execName string, amount int64) bool {
	signer := util.HexToPrivkey(fromKey)
	var tx *types.Transaction
	transfer := &cty.CoinsAction{}
...
func sendGuessStartTx(cfg *types.TuringchainConfig, topic, option, category, privKey string) (bool, []byte) {
...
func sendGuessBetTx(cfg *types.TuringchainConfig, gameID, option string, betsNum int64, privKey string) (bool, []byte) {
	signer := util.HexToPrivkey(privKey)
	var tx *types.Transaction
	action := &gty.GuessGameAction{}
...
func sendGuessStopTx(cfg *types.TuringchainConfig, gameID, privKey string) (bool, []byte) {
	signer := util.HexToPrivkey(privKey)
	var tx *types.Transaction
	action := &gty.GuessGameAction{}
...
func sendGuessAbortTx(cfg *types.TuringchainConfig, gameID, privKey string) (bool, []byte) {
	signer := util.HexToPrivkey(privKey)
	var tx *types.Transaction
	action := &gty.GuessGameAction{}
...
func sendGuessPublishTx(cfg *types.TuringchainConfig, gameID, result, privKey string) (bool, []byte) {
	signer := util.HexToPrivkey(privKey)
	var tx *types.Transaction
	action := &gty.GuessGameAction{}
...
func queryGuessByIds(gameIDs string) *gty.ReplyGuessGameInfos {
	...
	//ctx := jsonrpc.NewRPCCtx("http://"+types.Conf("config.rpc").GStr("jrpcBindAddr"), "Turingchain.Query", params, &res)
	ctx := jsonrpc.NewRPCCtx("http://127.0.0.1:9801", "Turingchain.Query", params, &res)
	ctx.Run()
	return &res
}
