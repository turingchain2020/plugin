...
import (
	...
	des "github.com/turingchain2020/plugin/plugin/dapp/storage/crypto"
	oty "github.com/turingchain2020/plugin/plugin/dapp/storage/types"
...
func QueryStorageByKey(stateDB dbm.KV, kvdb dbm.KVDB, key string, cfg *types.TuringchainConfig) (*oty.Storage, error) {
	exec := newStorage()
	q := queue.New("channel")
	q.SetConfig(cfg)
...
func QueryBatchStorageByKey(stateDB dbm.KV, kvdb dbm.KVDB, para proto.Message, cfg *types.TuringchainConfig) (*oty.BatchReplyStorage, error) {
	exec := newStorage()
	q := queue.New("channel")
	q.SetConfig(cfg)
	api, _ := client.New(q.Client(), nil)
...
func CreateTx(action string, message types.Message, priv string, cfg *types.TuringchainConfig) (*types.Transaction, error) {
