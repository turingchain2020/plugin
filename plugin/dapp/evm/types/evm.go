...
// NewType 新建EVM类型对象
func NewType(cfg *types.TuringchainConfig) *EvmType {
	c := &EvmType{}
	c.SetChild(c)
	c.SetConfig(cfg)
	return c
...
func createEvmTx(cfg *types.TuringchainConfig, param *CreateCallTx) (*types.Transaction, error) {
	if param == nil {
...
func createRawTx(cfg *types.TuringchainConfig, action proto.Message, name string, fee int64) (*types.Transaction, error) {
