...
import (
	...
	"github.com/turingchain2020/plugin/plugin/dapp/evm/executor/vm/gas"
	...
	"github.com/turingchain2020/plugin/plugin/dapp/evm/executor/vm/state"
...
func run(evm *EVM, contract *Contract, input []byte) (ret []byte, err error) {
	if contract.CodeAddr != nil {
		...
		//预编译分叉处理： turingchain中目前只存在拜占庭和最新的黄皮书v1版本（兼容伊斯坦布尔版本）
...
type EVM struct {
	...
	// turingchain配置
	cfg *types.TuringchainConfig
}

// NewEVM 创建一个新的EVM实例对象
// 在同一个节点中，一个EVM实例对象只服务于一个交易执行的生命周期
func NewEVM(ctx Context, statedb state.EVMStateDB, vmConfig Config, cfg *types.TuringchainConfig) *EVM {
	evm := &EVM{
		Context:     ctx,
		StateDB:     statedb,
		VMConfig:    vmConfig,
...
// 使用传入的部署代码创建新的合约；
// 目前turingchain为了保证账户安全，不允许合约中涉及到外部账户的转账操作，
// 所以，本步骤不接收转账金额参数
