...
import (
	...
	"github.com/turingchain2020/plugin/plugin/dapp/evm/commands/compiler"
	...
	common2 "github.com/turingchain2020/plugin/plugin/dapp/evm/executor/vm/common"
...
// transfer address format between ethereum and turingchain
...
func evmBalance(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Turingchain.GetBalance", params, &res)
...
func createContract(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Turingchain.SendTransaction", params, nil)
	ctx.RunWithoutMarshal()
}

func createEvmTx(cfg *types.TuringchainConfig, action proto.Message, execer, caller, addr, expire, rpcLaddr string, fee uint64) (string, error) {
	...
	err = client.Call("Turingchain.SignRawTx", unsignedTx, &res)
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		return "", err
	}
...
func createEvmTransferTx(cfg *types.TuringchainConfig, cmd *cobra.Command, caller, execName, expire, rpcLaddr string, amountInt64 int64, isWithdraw bool) (string, error) {
	paraName, _ := cmd.Flags().GetString("paraName")
...
func sendQuery(rpcAddr, funcName string, request types.Message, result proto.Message) bool {
	...
	err = jsonrpc.Call("Turingchain.Query", params, result)
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		return false
	}
