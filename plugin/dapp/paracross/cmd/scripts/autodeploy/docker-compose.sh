...
CLI="docker exec ${NODE1} /root/turingchain-cli"
...
TURINGCHAIN_CLI="turingchain-cli"
...
function para_init() {
    ...
    for auth in "${authAccount[@]}"; do
        tomlfile="turingchain.para.$index.toml"
...
function para_import_wallet() {
    ...
    ./$TURINGCHAIN_CLI --rpc_laddr "http://localhost:$port" seed save -p 1314fuzamei -s "tortoise main civil member grace happy century convince father cage beach hip maid merry rib"

    echo "=========== # unlock wallet ============="
    ./$TURINGCHAIN_CLI --rpc_laddr "http://localhost:$port" wallet unlock -p 1314fuzamei -t 0

    echo "=========== # import private key ============="
    echo "key: ${key}"
    ./$TURINGCHAIN_CLI --rpc_laddr "http://localhost:$port" account import_key -k "${key}" -l "paraAuthAccount"

    echo "=========== # close auto mining ============="
    ./$TURINGCHAIN_CLI --rpc_laddr "http://localhost:$port" wallet auto_mine -f 0

    echo "=========== # wallet status ============="
    ./$TURINGCHAIN_CLI --rpc_laddr "http://localhost:$port" wallet status
}

function para_unlock_wallet() {
    for ((i = 0; i < ${#authPort[@]}; i++)); do
        echo "=========== # para unlock wallet ${authPort[$i]}============="
        ./$TURINGCHAIN_CLI --rpc_laddr "http://localhost:${authPort[$i]}" wallet unlock -p 1314fuzamei -t 0
    done
...
function start() {
    ...
    #    ./turingchain-cli --rpc_laddr http://localhost:18901 block last_header
...
EOF
  ...
  parachain$i:
    ...
    environment:
      PARAFILE: "/root/turingchain.para.$i.toml"
    ports:
     - "1890$i:8901"
    volumes:
...
function create_build() {
    rm -rf $buildpath
    mkdir -p $buildpath
    cp turingchain* Dockerfile ./*.sh "$buildpath"/
    cd $buildpath
    create_yml
}
