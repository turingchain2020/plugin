#!/usr/bin/env bash
PARA_CLI="docker exec ${NODE3} /root/turingchain-cli --paraName user.p.para. --rpc_laddr http://localhost:8901"

PARA_CLI2="docker exec ${NODE2} /root/turingchain-cli --paraName user.p.para. --rpc_laddr http://localhost:8901"
PARA_CLI1="docker exec ${NODE1} /root/turingchain-cli --paraName user.p.para. --rpc_laddr http://localhost:8901"
PARA_CLI4="docker exec ${NODE4} /root/turingchain-cli --paraName user.p.para. --rpc_laddr http://localhost:8901"
PARA_CLI5="docker exec ${NODE5} /root/turingchain-cli --paraName user.p.game. --rpc_laddr http://localhost:8901"
MAIN_CLI="docker exec ${NODE3} /root/turingchain-cli"
...
function para_init() {
    para_set_toml turingchain.para33.toml "$PARANAME" "$1"
    para_set_toml turingchain.para32.toml "$PARANAME" "$1"
    para_set_toml turingchain.para31.toml "$PARANAME" "$1"
    para_set_toml turingchain.para30.toml "$PARANAME" "$1"

    sed -i $xsedfix 's/^authAccount=.*/authAccount="1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4"/g' turingchain.para33.toml
    sed -i $xsedfix 's/^authAccount=.*/authAccount="1JRNjdEqp4LJ5fqycUBm9ayCKSeeskgMKR"/g' turingchain.para32.toml
    sed -i $xsedfix 's/^authAccount=.*/authAccount="1NLHPEcbTWWxxU3dGUZBhayjrCHD3psX7k"/g' turingchain.para31.toml
    sed -i $xsedfix 's/^authAccount=.*/authAccount="1MCftFynyvG2F4ED5mdHYgziDxx6vDrScs"/g' turingchain.para30.toml

    para_set_toml turingchain.para29.toml "$PARANAME_GAME" "$1"
    sed -i $xsedfix 's/^authAccount=.*/authAccount="1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4"/g' turingchain.para29.toml
}

function para_set_toml() {
    cp turingchain.para.toml "${1}"
    local paraname="$2"
...
function para_transfer() {
    ...
    #token precreate
    txhash=$(para_configkey "${PARA_CLI}" "token-blacklist" "TRC")
    echo "txhash=$txhash"
...
function para_cross_transfer_withdraw() {
    ...
    while true; do
        ...
        acc_para=$(${PARA_CLI} asset balance -a 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv --asset_exec paracross --asset_symbol coins.trc | jq -r ".balance")
        ...
        if [ "${acc}" != "9.3000" ] || [ "${acc_para}" != "0.7000" ]; then
            ...
            if [ $times -le 0 ]; then
                ...
                ${PARA_CLI} asset balance -a 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv --asset_exec paracross --asset_symbol coins.trc
                exit 1
            fi
        else
...
function token_create_on_mainChain() {
    ...
    hash=$(${CLI} send config config_tx -c token-blacklist -o add -v TRC -k 0xc34b5d9d44ac7b754806f761d3d4d2c4fe5214f6b074c19f069c4f5c2a29c8cc)
    echo "${hash}"
...
function privacy_transfer_test() {
    ...
    ${1} send privacy pub2priv -a 9 -p fcbb75f2b96b6d41f301f2d1abc853d697818427819f412f8e4b4e12cacc0814d2c3914b27bea9151b8968ed1732bd241c8788a332b295b731aee8d39a060388 -e coins -s TRC -k 1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4
    ...
    check_privacy_utxo "${1}" 14KEKbYtKKQm4wMthSK9J4La4nAiidGozt coins TRC 9.0000

    echo "#privacy priv2priv, to=1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4"
    ${1} send privacy priv2priv -a 3 -p 5b0ff936ec2d2825a67a270e34d741d96bf6afe5d4b5692de0a1627f635fd0b3d7b14e44d3f8f7526030a7c59de482084161b441a5d66b483d80316e3b91482b -f 14KEKbYtKKQm4wMthSK9J4La4nAiidGozt -e coins -s TRC -k 14KEKbYtKKQm4wMthSK9J4La4nAiidGozt
    ...
    check_privacy_utxo "${1}" 1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4 coins TRC 3.0000

    echo "#privacy priv2pub, to=1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4"
    ${1} send privacy priv2pub -a 6 -t 1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4 -f 14KEKbYtKKQm4wMthSK9J4La4nAiidGozt -e coins -s TRC -k 14KEKbYtKKQm4wMthSK9J4La4nAiidGozt
    ...
    check_privacy_utxo "${1}" 14KEKbYtKKQm4wMthSK9J4La4nAiidGozt coins TRC 0.0000
}
