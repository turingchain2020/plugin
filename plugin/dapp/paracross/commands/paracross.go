...
func addCreateAssetTransferFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringP("symbol", "s", "", "default for trc, symbol for token")
}
...
func addCreateTransferToExecFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringP("symbol", "s", "coins.trc", "default for trc, symbol for token")
	cmd.MarkFlagRequired("symbol")
...
func addCreateCrossAssetTransferFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringP("symbol", "s", "", "asset symbol like trc")
	cmd.MarkFlagRequired("symbol")
...
func createNodeJoinTx(cmd *cobra.Command, args []string) {
	...
	rpcLaddr, _ := cmd.Flags().GetString("rpc_laddr")
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Turingchain.CreateTransaction", params, nil)
