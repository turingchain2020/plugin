...
func (suite *NodeManageTestSuite) SetupSuite() {
	...
	turingchainTestCfg.S("config.consensus.sub.para.MainForkParacrossCommitTx", int64(1))
	turingchainTestCfg.S("config.exec.sub.manage.superManager", []interface{}{Account12Q})
...
func voteTest(suite *NodeManageTestSuite, id string, join bool) {
	...
	config := &pt.ParaNodeAddrConfig{
		Title: turingchainTestCfg.GetTitle(),
		Op:    pt.ParaOpVote,
		Id:    id,
