...
import (
	...
	coins "github.com/turingchain2020/turingchain/system/dapp/coins/types"
...
coins+trc															mainTransfer	coins		trc
paracross+user.p.test1.coins.trc									paraWithdraw	coins		trc
user.p.test1.coins+trc												paraTransfer    coins		trc
user.p.test1.paracross+coins.trc									mainWithdraw	coins		trc
paracross+user.p.test1.coins.trc(->user.p.test2)					mainTransfer 	paracross   user.p.test1.coins.trc
user.p.test2.paracross+paracross.user.p.test1.coins.trc 			mainWithdraw	paracross   user.p.test1.coins.trc
注意:
1. user.p.test1.coins+trc只是对外表示平行链资产，真正执行器也是coins，因为account模型的mavl-coins-trc-　在主链和平行链都一样，平行链模型并不是mavl-user.p.test.coins-trc-
2. paracross执行器下的资产都是外来资产，在withdraw时候，真正的原生执行器是在symbol里面
　　a. 销毁资产　mavl-paracross-coins.trc-exec-addr(user)
　　b. 恢复资产　mavl-coins-trc-exec-addr{paracross}:addr{user}, 在原生coins执行器上恢复资产
*/
...
func (a *action) createAccount(cfg *types.TuringchainConfig, db db.KV, exec, symbol string) (*account.DB, error) {
...
func adaptNullAssetExec(transfer *pt.CrossAssetTransfer) {
	if transfer.AssetSymbol == "" {
		...
		transfer.AssetSymbol = SymbolTrc
		return
	}
