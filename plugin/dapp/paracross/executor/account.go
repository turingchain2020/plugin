...
// symbol: coins.trc, token.{TEST}
// 完整的帐号地址 mavl-{paracross}-coins.trc-{user-address} 不带title{paracross}
// 对应主链上paracross 子帐号 malv-coins-trc-exec-{Address(paracross)}:{Address(user.p.{guodun}.paracross)}
func NewParaAccount(cfg *types.TuringchainConfig, paraTitle, mainExecName, mainSymbol string, db db.KV) (*account.DB, error) {
...
func NewMainAccount(cfg *types.TuringchainConfig, paraTitle, paraExecName, paraSymbol string, db db.KV) (*account.DB, error) {
...
// mavl-conis-trc-exec-{para}1e:13DP8mVru5Rtu6CrjXQMvLsjvve3epRR1i
...
// 这样命名足以满足， 若要开启 平行链资产的转移， symbol 部分扩展为 symbol@host-title, 主链忽略 或添加 @turingchaincoin
