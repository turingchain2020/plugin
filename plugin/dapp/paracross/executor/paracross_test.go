...
import (
	...
	_ "github.com/turingchain2020/plugin/plugin/crypto/bls"
...
var (
	...
	turingchainTestMainCfg         = types.NewTuringchainConfig(strings.Replace(types.GetDefaultCfgstring(), "Title=\"local\"", "Title=\"test\"", 1))
)
...
func init() {
	...
	Init(pt.ParaX, turingchainTestCfg, nil)
}
...
func fillRawCommitTx(suite suite.Suite) (*types.Transaction, error) {
	...
	tx, err := pt.CreateRawCommitTx4MainChain(turingchainTestCfg, act, pt.GetExecName(turingchainTestCfg), 0)
	if err != nil {
...
func (s *VoteTestSuite) createVoteTx(status *pt.ParacrossNodeStatus, privFrom string) (*types.Transaction, error) {
	tx, err := pt.CreateRawMinerTx(turingchainTestCfg, &pt.ParacrossMinerAction{Status: status})
...
func createCrossCommitTx(s suite.Suite) (*types.Transaction, error) {
	...
	tx, err := pt.CreateRawCommitTx4MainChain(turingchainTestCfg, act, Title+pt.ParaX, 0)
	assert.Nil(s.T(), err, "create asset transfer failed")
	if err != nil {
		return nil, err
	}
...
func createTxsGroup(s suite.Suite, txs []*types.Transaction) ([]*types.Transaction, error) {
	group, err := types.CreateTxGroup(txs, turingchainTestCfg.GetMinTxFeeRate())
	if err != nil {
		return nil, err
	}
	err = group.Check(turingchainTestCfg, 0, turingchainTestCfg.GetMinTxFeeRate(), turingchainTestCfg.GetMaxTxFee())
	if err != nil {
		return nil, err
	}
...
func createParaNormalTx(s suite.Suite, privFrom string, to []byte) (*types.Transaction, error) {
	...
	tx, err := types.FormatTx(turingchainTestCfg, param.GetExecName(), tx)
	if err != nil {
		return nil, err
	}
