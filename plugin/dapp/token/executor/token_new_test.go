...
var (
	mainNetgrpcAddr = "localhost:9672"
	ParaNetgrpcAddr = "localhost:8902"
	mainClient      types.TuringchainClient
	paraClient      types.TuringchainClient
	r               *rand.Rand
...
func init() {
	...
	mainClient = types.NewTuringchainClient(conn)

	conn, err = grpc.Dial(ParaNetgrpcAddr, grpc.WithInsecure())
	if err != nil {
		panic(err)
	}
	paraClient = types.NewTuringchainClient(conn)
