...
var (
	...
	CoinsTrcDailylimit    uint64 = 100
	NewCoinsTrcDailylimit uint64 = 10
	PrintFlag                    = false
	InAmount              int64  = 10
	OutAmount             int64  = 5
	chainTestCfg                 = types.NewTuringchainConfig(types.GetDefaultCfgstring())
)
...
func TestMultiSigAccCreate(t *testing.T) {
	...
	//modify assets DailyLimit NewCoinsTrcDailylimit
	//t.Log("------testMultiSigAccDailyLimitModify------")
	testMultiSigAccDailyLimitModify(t, driver, env, multiSigAddr)

	//confirmtx  assets DailyLimit NewCoinsTrcDailylimit
...
func testMultiSigAccDailyLimitModify(t *testing.T, driver drivers.Driver, env execEnv, multiSigAddr string) {
	assetsDailyLimit := &mty.SymbolDailyLimit{
		Symbol:     Symbol,
		Execer:     Asset,
		DailyLimit: NewCoinsTrcDailylimit,
	}
	params := &mty.MultiSigAccOperate{
		MultiSigAccAddr: multiSigAddr,
	...
	assert.Equal(t, receiptMultiSigDailyLimitOperate.PrevDailyLimit.DailyLimit, CoinsTrcDailylimit)

	assert.Equal(t, receiptMultiSigDailyLimitOperate.PrevDailyLimit.Symbol, Symbol)
	assert.Equal(t, receiptMultiSigDailyLimitOperate.PrevDailyLimit.Execer, Asset)
	assert.Equal(t, receiptMultiSigDailyLimitOperate.CurDailyLimit.DailyLimit, NewCoinsTrcDailylimit)
...
//[symbol:"trc" execer:"coins" dailyLimit:10 ]
//6
// 4}
