...
function kvcreator(dtrcpe) {
	...
	this.list = listdb
    if (dtrcpe == "exec" || dtrcpe == "init") {
        this.getdb = this.getstate
    } else if (dtrcpe == "local") {
        this.getdb = this.getlocal
    } else if (dtrcpe == "query") {
		this.getdb = this.getlocal
	} else {
		throw new Error("turingchain.js: dtrcpe error")
	}
}
...
function callcode(context, f, args, loglist) {
    ...
    if (farr.length !=  2) {
        throw new Error("turingchain.js: invalid function name format")
    }
    var prefix = farr[0]
    var funcname = farr[1]
    var runobj = {}
    ...
    if (!Array.isArray(loglist)) {
        throw new Error("turingchain.js: loglist must be array")
    }
    ...
    if (prefix == "exec") {
        ...
        throw new Error("turingchain.js: invalid function prefix format")
    }
    var arg = JSON.parse(args)
    if (typeof runobj[funcname] != "function") {
        throw new Error("turingchain.js: invalid function name not found->" + funcname)
    }
...
function ExecInit() {
    this.acc = new account(this.kvc, "coins", "trc")
}
